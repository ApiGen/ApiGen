...
final class AnalyzeTaskX implements Task
{
	 ...
	 * @param Node[] $nodes
	 */
	private function processNodes(array $nodes): Iterator
	{
		foreach ($nodes as $node) {
			if ($node instanceof Node\Stmt\Namespace_) {
				yield from $this->processNodes($node->stmts);
				...
				if ($node instanceof Node\Stmt\Class_) {
					...
					$info->extends = $node->extends ? $node->extends->toLowerString() : null;
					$info->implements = array_map('strtolower', array_map([$this, 'processName'], $node->implements));
					$info->dependencies += $info->extends ? [$info->extends => true] : [];
					$info->dependencies += array_fill_keys($info->implements, true);

				} elseif ($node instanceof Node\Stmt\Interface_) {
					$info = new InterfaceInfo($name);
					$info->extends = array_map('strtolower', array_map([$this, 'processName'], $node->extends));
					$info->dependencies += array_fill_keys($info->extends, true);
	...
	private function processName(?Node\Name $name): ?string
	{
