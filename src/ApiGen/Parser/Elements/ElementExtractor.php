...
class ElementExtractor extends Nette\Object
{
	 ...
	 * @return array[]
	 */
	public function extractElementsByAnnotation($annotation)
	{
		...
		foreach ($this->elementStorage->getElements() as $type => $elementList) {
			...
			foreach ($elementList as $class) {
				...
				$elements[Elements::METHODS] = $this->extractByAnnotationAndJoin(
					$class->getOwnMethods(), $annotation, $elements[Elements::METHODS]
				);

				$elements[Elements::CONSTANTS] = $this->extractByAnnotationAndJoin(
					$class->getOwnConstants(), $annotation, $elements[Elements::CONSTANTS]
				);

				$elements[Elements::PROPERTIES] = $this->extractByAnnotationAndJoin(
	 ...
	 * @return array[]
	 */
	private function extractByAnnotationAndJoin($elements, $annotation, $storage)
	{
		$foundElements = $this->elementFilter->filterByAnnotation($elements, $annotation);
		$storage = array_merge($storage, array_values($foundElements));
		return $storage;
	}
