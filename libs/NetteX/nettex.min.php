...
/**
 * NetteX Framework (version 2.0-dev released on 2011-06-13, http://nette.org)
 *
...
@header('X-Powered-By: NetteX Framework');

define('NETTEX', TRUE);
define('NETTEX_DIR', __DIR__);
define('NETTEX_VERSION_ID', 20000);
define('NETTEX_PACKAGE', '5.3');

}

namespace NetteX\Diagnostics {
...
interface IResponse
{
	...
	function send(NetteX\Http\IRequest $httpRequest, NetteX\Http\IResponse $httpResponse);
...
interface IRouter
{
	...
	function match(NetteX\Http\IRequest $httpRequest);

	function constructUrl(Request $appRequest, NetteX\Http\Url $refUrl);
...
namespace NetteX\ComponentModel {
...
namespace NetteX\Application\UI {
...
namespace NetteX\Caching\Storages {
...
interface IUser
{
	...
	function setAuthenticator(NetteX\Security\IAuthenticator $handler);
	...
	function setAuthorizator(NetteX\Security\IAuthorizator $handler);
...
final class LimitedScope
{
	...
	{
		throw new NetteX\StaticClassException;
	}
	...
	static function evaluate()
	{
		...
		if ($res === FALSE && ($error = error_get_last()) && $error['type'] === E_PARSE) {
			throw new NetteX\FatalErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'], NULL);
		}
		return $res;
	}
...
namespace NetteX\Loaders {

use NetteX;

abstract class AutoLoader extends NetteX\Object
{
	...
	final static function load($type)
	{
		foreach (func_get_args() as $type) {
			if (!class_exists($type)) {
				throw new NetteX\InvalidStateException("Unable to load class or interface '$type'.");
			}
		}
	}
	...
	function register()
	{
		if (!function_exists('spl_autoload_register')) {
			throw new NetteX\NotSupportedException('spl_autoload does not exist in this PHP installation.');
		}
...
final class Debugger
{
	...
	static function _init()
	{
		...
		self::$blueScreen->addPanel(function($e) {
			if ($e instanceof NetteX\Templating\FilterException) {
				return array(
					'tab' => 'Template',
	...
	static function enable($mode = NULL, $logDirectory = NULL, $email = NULL)
	{
		...
		if (self::$productionMode === self::DETECT) {
			if (class_exists('NetteX\Environment')) {
				self::$productionMode = NetteX\Environment::isProduction();
		...
		if (is_string($logDirectory)) {
			...
			if (self::$logDirectory === FALSE) {
				throw new NetteX\DirectoryNotFoundException("Directory '$logDirectory' is not found.");
			}
		...
		if (function_exists('ini_set')) {
			...
			throw new NetteX\NotSupportedException('Function ini_set() must be enabled.');
		}

		if ($email) {
			if (!is_string($email)) {
				throw new NetteX\InvalidArgumentException('Email address must be a string.');
			}
			self::$email = $email;
		}
	...
	static function log($message, $priority = self::INFO)
	{
		if (self::$logDirectory === FALSE) {
			...
			throw new NetteX\InvalidStateException('Logging directory is not specified in NetteX\Diagnostics\Debugger::$logDirectory.');
		}

		if ($message instanceof \Exception) {
			$exception = $message;
			$message = "PHP Fatal error: "
				. ($message instanceof NetteX\FatalErrorException
	...
	static function _shutdownHandler()
	{
		...
		if (isset($types[$error['type']])) {
			self::_exceptionHandler(new NetteX\FatalErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'], NULL));
		}
...
<?php
	...
	static function _errorHandler($severity, $message, $file, $line, $context)
	{
		...
		if ($severity === E_RECOVERABLE_ERROR || $severity === E_USER_ERROR) {
			throw new NetteX\FatalErrorException($message, 0, $severity, $file, $line, $context);

		} elseif (($severity & error_reporting()) !== $severity) {
			return FALSE;

		} elseif (self::$strictMode && !self::$productionMode) {
			self::_exceptionHandler(new NetteX\FatalErrorException($message, 0, $severity, $file, $line, $context));
		}
	...
	static function dumpX($var, $return = FALSE)
	{
...
class Logger extends NetteX\Object
{
	const DEBUG = 'debug',
		INFO = 'info',
		WARNING = 'warning',
	...
	function log($message, $priority = self::INFO)
	{
		if (!is_dir($this->directory)) {
			throw new NetteX\DirectoryNotFoundException("Directory '$this->directory' is not found or is not directory.");
		}
...
class FireLogger extends NetteX\Object
{
	const DEBUG = 'debug',
		INFO = 'info',
		WARNING = 'warning',
	...
	static function log($message, $priority = self::DEBUG)
	{
		...
		if (isset($args[0]) && $args[0] instanceof \Exception) {
			$e = array_shift($args);
			$trace = $e->getTrace();
			if (isset($trace[0]['class']) && $trace[0]['class'] === 'NetteX\Diagnostics\Debugger'
			...
			$trace = debug_backtrace();
			if (isset($trace[1]['class']) && $trace[1]['class'] === 'NetteX\Diagnostics\Debugger'
	...
	private static function jsonDump(&$var, $level = 0)
	{
		if (is_bool($var) || is_null($var) || is_int($var) || is_float($var)) {
			...
			return NetteX\Utils\Strings::fixEncoding($var);
...
class BlueScreen extends NetteX\Object
{
...
$title = ($exception instanceof NetteX\FatalErrorException && isset($errorTypes[$exception->getSeverity()])) ? $errorTypes[$exception->getSeverity()] : get_class($exception);

$expandPath = NETTEX_DIR . DIRECTORY_SEPARATOR;
$counter = 0;
...
<div id="netteBluescreen">
	...
	><div>
		...
		<ul>
			...
			<?php if (class_exists('NetteX\Framework')): ?><li><?php echo htmlSpecialChars('NetteX Framework ' . NetteX\Framework::VERSION) ?> <i>(revision <?php echo htmlSpecialChars(NetteX\Framework::REVISION) ?>)</i></li><?php endif ?>
		</ul>
	</div>
</div>
...
class Bar extends NetteX\Object
{
...
(function (onloadOrig) {
	window.onload = function() {
		...
		debug.innerHTML = <?php echo json_encode(NetteX\Utils\Strings::fixEncoding($output)) ?>;
...
final class DefaultBarPanel extends NetteX\Object implements IBarPanel
{
	private $id;
...
class Application extends NetteX\Object
{
	...
	function __construct(NetteX\DI\IContainer $context)
	{
		$this->context = $context;
	}
	...
	function run()
	{
		...
		if ($this->allowedMethods) {
			...
			if (!in_array($method, $this->allowedMethods, TRUE)) {
				$httpResponse->setCode(NetteX\Http\IResponse::S501_NOT_IMPLEMENTED);
			...
			try {
				...
				if (!$repeatedError && $this->errorPresenter) {
					...
					if ($e instanceof BadRequestException) {
						...
						$code = 500;
						NetteX\Diagnostics\Debugger::log($e, NetteX\Diagnostics\Debugger::ERROR);
					}
...
<?php
	...
	{
		$session = $this->context->session->getSection('NetteX.Application/requests');
		do {
			$key = NetteX\Utils\Strings::random(5);
...
namespace NetteX\Application\Diagnostics {

use NetteX;use NetteX\Application\Routers;use NetteX\Application\UI\Presenter;
use NetteX\Diagnostics\Debugger;

class RoutingPanel extends NetteX\Object implements NetteX\Diagnostics\IBarPanel
{
	...
	static function initialize(NetteX\Application\Application $application, NetteX\Http\IRequest $httpRequest)
	{
		Debugger::$bar->addPanel(new self($application->getRouter(), $httpRequest));
		Debugger::$blueScreen->addPanel(function($e) use($application) {
			if ($e === NULL) {
				return array(
					'tab' => 'NetteX Application',
					'panel' => '<h3>Requests</h3>' . NetteX\Diagnostics\Helpers::clickableDump($application->getRequests())
						. '<h3>Presenter</h3>' . NetteX\Diagnostics\Helpers::clickableDump($application->getPresenter())
				);
			}
		});
	}

	function __construct(NetteX\Application\IRouter $router, NetteX\Http\IRequest $httpRequest)
	{
		$this->router = $router;
...
namespace NetteModuleX {

use NetteX;use NetteX\Application;use NetteX\Application\Responses;use NetteX\Http;

class MicroPresenter extends NetteX\Object implements Application\IPresenter
{
	...
	function run(Application\Request $request)
	{
		...
		$response = callbackX($params['callback'])->invokeNamedArgs($params);
		...
		if (is_array($response)) {
			if ($response instanceof \SplFileInfo) {
				$response = $this->createTemplate('NetteX\Templating\FileTemplate')
					->setParams($response[1])->setFile($response[0]);
			} else {
				$response = $this->createTemplate('NetteX\Templating\Template')
					->setParams($response[1])->setSource($response[0]);
			}
		}
		if ($response instanceof NetteX\Templating\ITemplate) {
	...
	{
		$template = $class ? new $class : new NetteX\Templating\FileTemplate;
		...
		$template->onPrepareFilters[] = function($template) use($latteFactory, $context) {
			$template->registerFilter($latteFactory ? $latteFactory() : new NetteX\Latte\Engine);
		};
		return $template;
	}
	...
	function setContext(NetteX\DI\IContainer $context)
	{
		$this->context = $context;
		return $this;
	}
...
class PresenterFactory implements IPresenterFactory
{
	...
	function __construct($baseDir, NetteX\DI\IContainer $context)
	{
	...
	function getPresenterClass(& $name)
	{
		...
		if (!is_string($name) || !NetteX\Utils\Strings::match($name, "#^[a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff:]*$#")) {
		...
		if (!class_exists($class)) {
			...
			if (is_file($file) && is_readable($file)) {
				NetteX\Utils\LimitedScope::load($file);
			}
		...
		$reflection = new NetteX\Reflection\ClassType($class);
		$class = $reflection->getName();

		if (!$reflection->implementsInterface('NetteX\Application\IPresenter')) {
			throw new InvalidPresenterException("Cannot load presenter '$name', class '$class' is not NetteX\\Application\\IPresenter implementor.");
		}
...
final class Request extends NetteX\FreezableObject
{
...
namespace NetteX\Application\Responses {

use NetteX;

class FileResponse extends NetteX\Object implements NetteX\Application\IResponse
{
	...
	{
		if (!is_file($file)) {
			throw new NetteX\Application\BadRequestException("File '$file' doesn't exist.");
		}
...
class ForwardResponse extends NetteX\Object implements NetteX\Application\IResponse
{

	private $request;

	function __construct(NetteX\Application\Request $request)
	{
		$this->request = $request;
	}
...
class JsonResponse extends NetteX\Object implements NetteX\Application\IResponse
{
	...
	function __construct($payload, $contentType = NULL)
	{
		if (!is_array($payload) && !is_object($payload)) {
			throw new NetteX\InvalidArgumentException("Payload must be array or object class, " . gettype($payload) . " given.");
		}
	...
	function send(NetteX\Http\IRequest $httpRequest, NetteX\Http\IResponse $httpResponse)
	{
		...
		echo NetteX\Utils\Json::encode($this->payload);
	}
...
class RedirectResponse extends NetteX\Object implements NetteX\Application\IResponse
{
...
class TextResponse extends NetteX\Object implements NetteX\Application\IResponse
{
	...
	function send(NetteX\Http\IRequest $httpRequest, NetteX\Http\IResponse $httpResponse)
	{
		if ($this->source instanceof NetteX\Templating\ITemplate) {
			$this->source->render();
...
namespace NetteX\Application\Routers {

use NetteX;use NetteX\Application;

class CliRouter extends NetteX\Object implements Application\IRouter
{
	...
	function match(NetteX\Http\IRequest $httpRequest)
	{
		...
		if (!isset($params[self::PRESENTER_KEY])) {
			throw new NetteX\InvalidStateException('Missing presenter & action in route definition.');
		}
	...
	function constructUrl(Application\Request $appRequest, NetteX\Http\Url $refUrl)
	{
		return NULL;
	}
...
use NetteX\Utils\Strings;

class Route extends NetteX\Object implements Application\IRouter
{
	...
	function __construct($mask, $metadata = array(), $flags = 0)
	{
		if (is_string($metadata)) {
			$a = strrpos($metadata, ':');
			if (!$a) {
				throw new NetteX\InvalidArgumentException("Second argument must be array or string in format Presenter:action, '$metadata' given.");
			}
			$metadata = array(
		...
		} elseif ($metadata instanceof \Closure || $metadata instanceof NetteX\Callback) {
			$metadata = array(
				self::PRESENTER_KEY => 'NetteX:Micro',
				'callback' => $metadata,
			);
		}
	...
	function match(NetteX\Http\IRequest $httpRequest)
	{
		...
		if (!isset($params[self::PRESENTER_KEY])) {
			throw new NetteX\InvalidStateException('Missing presenter in route definition.');
		}
		if (isset($this->metadata[self::MODULE_KEY])) {
			if (!isset($params[self::MODULE_KEY])) {
				throw new NetteX\InvalidStateException('Missing module in route definition.');
			}
	...
	private function setMask($mask, array $metadata)
	{
			...
			foreach ($matches as $match) {
				...
				if ($class !== '') {
					if (!isset(self::$styles[$class])) {
						throw new NetteX\InvalidStateException("Parameter '$name' has '$class' flag, but Route::\$styles['$class'] is not set.");
					}
		...
		do {
			...
			if ($part === '[' || $part === ']' || $part === '[!') {
				$brackets += $part[0] === '[' ? -1 : 1;
				if ($brackets < 0) {
					throw new NetteX\InvalidArgumentException("Unexpected '$part' in mask '$mask'.");
				}
			...
			if (preg_match('#[^a-z0-9_-]#i', $name)) {
				throw new NetteX\InvalidArgumentException("Parameter name must be alphanumeric string due to limitations of PCRE, '$name' given.");
			}
		...
		if ($brackets) {
			throw new NetteX\InvalidArgumentException("Missing closing ']' in mask '$mask'.");
		}
	...
	static function addStyle($style, $parent = '#')
	{
		if (isset(self::$styles[$style])) {
			throw new NetteX\InvalidArgumentException("Style '$style' already exists.");
		}

		if ($parent !== NULL) {
			if (!isset(self::$styles[$parent])) {
				throw new NetteX\InvalidArgumentException("Parent style '$parent' doesn't exist.");
			}
	...
	{
		if (!isset(self::$styles[$style])) {
			throw new NetteX\InvalidArgumentException("Style '$style' doesn't exist.");
		}
...
class RouteList extends NetteX\ArrayList implements NetteX\Application\IRouter
{
	...
	function constructUrl(NetteX\Application\Request $appRequest, NetteX\Http\Url $refUrl)
	{
	...
	{
		if (!$route instanceof NetteX\Application\IRouter) {
			throw new NetteX\InvalidArgumentException("Argument must be IRouter descendant.");
		}
...
use NetteX\Application;

class SimpleRouter extends NetteX\Object implements Application\IRouter
{
	const PRESENTER_KEY = 'presenter';
	const MODULE_KEY = 'module';
	...
	function __construct($defaults = array(), $flags = 0)
	{
		if (is_string($defaults)) {
			$a = strrpos($defaults, ':');
			if (!$a) {
				throw new NetteX\InvalidArgumentException("Argument must be array or string in format Presenter:action, '$defaults' given.");
			}
			$defaults = array(
	...
	function match(NetteX\Http\IRequest $httpRequest)
	{
		...
		if (!isset($params[self::PRESENTER_KEY])) {
			throw new NetteX\InvalidStateException('Missing presenter.');
		}
...
class BadSignalException extends NetteX\Application\BadRequestException
{
...
abstract class Component extends NetteX\Object implements IComponent
{
	...
	function lookup($type, $need = TRUE)
	{
		...
		if ($need && $this->monitors[$type][0] === NULL) {
			throw new NetteX\InvalidStateException("Component '$this->name' is not attached to '$type'.");
		}
	...
	function setParent(IContainer $parent = NULL, $name = NULL)
	{
		...
		if ($this->parent !== NULL && $parent !== NULL) {
			throw new NetteX\InvalidStateException("Component '$this->name' already has a parent.");
		}
	...
	{
		throw new NetteX\NotImplementedException;
	}
...
class Container extends Component implements IContainer
{
	...
	function addComponent(IComponent $component, $name, $insertBefore = NULL)
	{
		...
		if (is_int($name)) {
			...
			throw new NetteX\InvalidArgumentException("Component name must be integer or string, " . gettype($name) . " given.");

		} elseif (!preg_match('#^[a-zA-Z0-9_]+$#', $name)) {
			throw new NetteX\InvalidArgumentException("Component name must be non-empty alphanumeric string, '$name' given.");
		}

		if (isset($this->components[$name])) {
			throw new NetteX\InvalidStateException("Component with name '$name' already exists.");
		}

		$obj = $this;
		do {
			if ($obj === $component) {
				throw new NetteX\InvalidStateException("Circular reference detected while adding component '$name'.");
			}
			$obj = $obj->getParent();
		} while ($obj !== NULL);
	...
	function removeComponent(IComponent $component)
	{
		...
		if (!isset($this->components[$name]) || $this->components[$name] !== $component) {
			throw new NetteX\InvalidArgumentException("Component named '$name' is not located in this container.");
		}
	...
	final function getComponent($name, $need = TRUE)
	{
		if (is_int($name)) {
			...
			if ($name === '') {
				throw new NetteX\InvalidArgumentException("Component or subcomponent name must not be empty string.");
			}
		}
		...
		if (isset($this->components[$name])) {
			if (!isset($ext)) {
				...
				throw new NetteX\InvalidArgumentException("Component with name '$name' is not container and cannot have '$ext' component.");
			}

		} elseif ($need) {
			throw new NetteX\InvalidArgumentException("Component with name '$name' does not exist.");
		}
	}
	...
	protected function createComponent($name)
	{
		...
		if ($ucname !== $name && method_exists($this, $method) && $this->getReflection()->getMethod($method)->getName() === $method) {
			...
			if (!$component instanceof IComponent && !isset($this->components[$name])) {
				$class = get_class($this);
				throw new NetteX\UnexpectedValueException("Method $class::$method() did not return or create the desired component.");
			}
			return $component;
		}
	}
	...
	final function getComponents($deep = FALSE, $filterType = NULL)
	{
		...
		if ($filterType) {
			$iterator = new NetteX\Iterators\InstanceFilter($iterator, $filterType);
		}
		return $iterator;
	}
...
abstract class PresenterComponent extends NetteX\ComponentModel\Container implements ISignalReceiver, IStatePersistent, \ArrayAccess
{

	protected $params = array();

	function __construct(NetteX\ComponentModel\IContainer $parent = NULL, $name = NULL)
	{
		$this->monitor('NetteX\Application\UI\Presenter');
		parent::__construct($parent, $name);
	}

	function getPresenter($need = TRUE)
	{
		return $this->lookup('NetteX\Application\UI\Presenter', $need);
	}

	function getUniqueId()
	{
		return $this->lookupPath('NetteX\Application\UI\Presenter', TRUE);
	}
	...
	function saveState(array & $params, $reflection = NULL)
	{
		...
		foreach ($reflection->getPersistentParams() as $nm => $meta) {
			...
			if (is_object($val)) {
				$class = get_class($this);
				throw new NetteX\InvalidStateException("Persistent parameter must be scalar or array, $class::\$$nm is " . gettype($val));
	...
	{
		$rc = new NetteX\Reflection\ClassType(get_called_class());
		$params = array();
...
abstract class Control extends PresenterComponent implements IPartiallyRenderable
{
	...
	final function getTemplate()
	{
		if ($this->template === NULL) {
			...
			if (!$value instanceof NetteX\Templating\ITemplate && $value !== NULL) {
				$class2 = get_class($value); $class = get_class($this);
				throw new NetteX\UnexpectedValueException("Object returned by $class::createTemplate() must be instance of NetteX\\Templating\\ITemplate, '$class2' given.");
			}
			$this->template = $value;
		}
	...
	protected function createTemplate($class = NULL)
	{
		...
		$template->onPrepareFilters[] = callbackX($this, 'templatePrepareFilters');
		$template->registerHelperLoader('NetteX\Templating\DefaultHelpers::loader');
	...
	{
		$template->registerFilter(new NetteX\Latte\Engine);
	}
...
class Container extends NetteX\ComponentModel\Container implements \ArrayAccess
{
	...
	function setValues($values, $erase = FALSE)
	{
		if ($values instanceof \Traversable) {
			...
			throw new NetteX\InvalidArgumentException("First parameter must be an array, " . gettype($values) ." given.");
		}
	...
	function addComponent(NetteX\ComponentModel\IComponent $component, $name, $insertBefore = NULL)
	{
	...
	{
		return $this->getComponents(TRUE, 'NetteX\Forms\IControl');
	}

	function getForm($need = TRUE)
	{
		return $this->lookup('NetteX\Forms\Form', $need);
	}
	...
	{
		throw new NetteX\NotImplementedException('Form cloning is not supported yet.');
	}
...
class Form extends Container
{
	...
	const PROTECTION = 'NetteX\Forms\Controls\HiddenField::validateEqual';
	...
	{
		$this->element = NetteX\Utils\Html::el('form');
		$this->element->action = '';
	...
	protected function attached($obj)
	{
		if ($obj instanceof self) {
			throw new NetteX\InvalidStateException('Nested forms are forbidden.');
		}
	}
	...
	function setMethod($method)
	{
		if ($this->httpData !== NULL) {
			throw new NetteX\InvalidStateException(__METHOD__ . '() must be called until the form is empty.');
		}
	...
	{
		$session = $this->getSession()->getSection('NetteX.Forms.Form/CSRF');
		$key = "key$timeout";
		if (isset($session->$key)) {
			$token = $session->$key;
		} else {
			$session->$key = $token = NetteX\Utils\Strings::random();
		}
	...
	function removeGroup($name)
	{
		if (is_string($name) && isset($this->groups[$name])) {
			...
			throw new NetteX\InvalidArgumentException("Group not found in form '$this->name'");
		}
	...
	function setTranslator(NetteX\Localization\ITranslator $translator = NULL)
	{
	...
	final function getHttpData()
	{
		if ($this->httpData === NULL) {
			if (!$this->isAnchored()) {
				throw new NetteX\InvalidStateException('Form is not anchored and therefore can not determine whether it was submitted.');
			}
	...
	protected function receiveHttpData()
	{
		...
		if ($httpRequest->isMethod('post')) {
			$data = NetteX\Utils\Arrays::mergeTree($httpRequest->getPost(), $httpRequest->getFiles());
		} else {
	...
	function __toString()
	{
		try {
			...
			if (func_get_args() && func_get_arg(0)) {
				...
				NetteX\Diagnostics\Debugger::toStringException($e);
			}
		}
	}
	...
	{
		return NetteX\Environment::getHttpRequest();
	}

	protected function getSession()
	{
		return NetteX\Environment::getSession();
	}
...
class Form extends NetteX\Forms\Form implements ISignalReceiver
{
	...
	{
		if ($presenter instanceof Presenter) {
			$name = $this->lookupPath('NetteX\Application\UI\Presenter');
	...
	protected function receiveHttpData()
	{
		...
		if ($isPost) {
			return NetteX\Utils\Arrays::mergeTree($request->getPost(), $request->getFiles());
		} else {
...
class Link extends NetteX\Object
{
...
use NetteX\Application;use NetteX\Application\Responses;use NetteX\Http;use NetteX\Reflection;
...
abstract class Presenter extends Control implements Application\IPresenter
{
	...
	function run(Application\Request $request)
	{
		try {
			...
			if (!$this->startupCheck) {
				...
				throw new NetteX\InvalidStateException("Method $class::startup() or its descendant doesn't call parent::startup().");
			}
	...
	{
		if ($component instanceof NetteX\ComponentModel\Component) {
	...
	{
		if (NetteX\Utils\Strings::match($action, "#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*$#")) {
			$this->action = $action;
			$this->view = $action;
	...
	function sendTemplate()
	{
		...
		if ($template instanceof NetteX\Templating\IFileTemplate && !$template->getFile()) {
		...
		if ($this->layout !== FALSE) {
			...
			if (empty($template->layout) && $this->layout !== NULL) {
				$file = preg_replace('#^.*([/\\\\].{1,70})$#U', "\xE2\x80\xA6\$1", reset($files));
				$file = strtr($file, '/', DIRECTORY_SEPARATOR);
				throw new NetteX\FileNotFoundException("Layout not found. Missing template '$file'.");
			}
		}
	...
	private function getGlobalState($forClass = NULL)
	{
		...
		if ($this->globalState === NULL) {
			...
			$iterator = $this->getComponents(TRUE, 'NetteX\Application\UI\IStatePersistent');
	...
	function hasFlashSession()
	{
		return !empty($this->params[self::FLASH_KEY])
			&& $this->getSession()->hasSection('NetteX.Application.Flash/' . $this->params[self::FLASH_KEY]);
	}

	function getFlashSession()
	{
		if (empty($this->params[self::FLASH_KEY])) {
			$this->params[self::FLASH_KEY] = NetteX\Utils\Strings::random(4);
		}
		return $this->getSession('NetteX.Application.Flash/' . $this->params[self::FLASH_KEY]);
	}
...
use NetteX;use NetteX\ObjectMixin;
...
class ClassType extends \ReflectionClass
{
	...
	function setExtensionMethod($name, $callback)
	{
		...
		$l[strtolower($this->getName())] = callbackX($callback);
		$l[''] = NULL;
		return $this;
	}
...
class PresenterComponentReflection extends NetteX\Reflection\ClassType
{
	...
	function getPersistentParams($class = NULL)
	{
		...
		$params = array();
		if (is_subclass_of($class, 'NetteX\Application\UI\PresenterComponent')) {
	...
	function getPersistentComponents()
	{
		...
		$components = array();
		if (is_subclass_of($class, 'NetteX\Application\UI\Presenter')) {
	...
	function hasCallableMethod($method)
	{
		...
		if ($cache === NULL) try {
			$cache = FALSE;
			$rm = NetteX\Reflection\Method::from($class, $method);
...
class Cache extends NetteX\Object implements \ArrayAccess
{
	...
	function save($key, $data, array $dp = NULL)
	{
		...
		if (isset($dp[Cache::EXPIRATION])) {
			$dp[Cache::EXPIRATION] = NetteX\DateTime::from($dp[Cache::EXPIRATION])->format('U') - time();
		}
		...
		if ($data instanceof NetteX\Callback || $data instanceof \Closure) {
			NetteX\Utils\CriticalSection::enter();
			$data = $data->__invoke();
			NetteX\Utils\CriticalSection::leave();
		}

		if (is_object($data)) {
			$dp[self::CALLBACKS][] = array(array(__CLASS__, 'checkSerializationVersion'), get_class($data),
				NetteX\Reflection\ClassType::from($data)->getAnnotation('serializationVersion'));
		}
	...
	{
		return NetteX\Reflection\ClassType::from($class)->getAnnotation('serializationVersion') === $value;
	}

}

class OutputHelper extends NetteX\Object
{
	...
	{
		if ($this->cache === NULL) {
			throw new NetteX\InvalidStateException('Output cache has already been saved.');
		}
...
class DevNullStorage extends NetteX\Object implements NetteX\Caching\IStorage
{
...
use NetteX\Caching\Cache;

class FileJournal extends NetteX\Object implements IJournal
{
	...
	function __construct($dir)
	{
		...
		if (!file_exists($this->file)) {
			...
			if (!$init) {
				clearstatcache();
				if (!file_exists($this->file)) {
					throw new NetteX\InvalidStateException("Cannot create journal file $this->file.");
				}
			} else {
				$writen = fwrite($init, pack('N2', self::FILE_MAGIC, $this->lastNode));
				fclose($init);
				if ($writen !== self::INT32_SIZE * 2) {
					throw new NetteX\InvalidStateException("Cannot write journal header.");
				}
			}
		}
		...
		if (!$this->handle) {
			throw new NetteX\InvalidStateException("Cannot open journal file '$this->file'.");
		}

		if (!flock($this->handle, LOCK_SH)) {
			throw new NetteX\InvalidStateException('Cannot acquire shared lock on journal.');
		}
		...
		if ($fileMagic !== self::FILE_MAGIC) {
			fclose($this->handle);
			$this->handle = false;
			throw new NetteX\InvalidStateException("Malformed journal file '$this->file'.");
		}
	}
	...
	private function cleanPriority($priority, array &$toDelete)
	{
		...
		while ($nodeInfo[self::PREV_NODE] !== -1) {
			...
			if ($node === FALSE) {
				if (self::$debug) {
					throw new NetteX\InvalidStateException("Cannot load node number $nodeId.");
				}
				break;
			}
	...
	private function cleanLinks(array $data, array &$toDelete)
	{
		...
		while ($i < $max) {
			...
			do {
				...
				if (!isset($node[$link])) {
					if (self::$debug) {
						throw new NetteX\InvalidStateException("Link with ID $searchLink is not in node ". ($nodeId) . '.');
					}
					continue;
		...
		foreach ($toDeleteFromIndex as $type => $toDelete) {
			...
			while (!empty($toDelete)) {
				...
				if (!isset($masterNode[$searchKey])) {
					if (self::$debug) {
						throw new NetteX\InvalidStateException('Bad index.');
					}
	...
	private function mergeIndexData(array $data)
	{
		while (isset($data[self::INDEX_DATA])) {
			...
			if ($childNode === FALSE) {
				if (self::$debug) {
					throw new NetteX\InvalidStateException("Cannot load node number $id.");
				}
				break;
			}
		...
		while ($nextNodeId && !empty($links)) {
			...
			if (empty($node[self::INDEX_DATA]) || (count($node[self::INDEX_DATA]) === 1 && $nextNodeId)) {
				if ($prev === -1) {
					...
					if ($prevNode === FALSE) {
						if (self::$debug) {
							throw new NetteX\InvalidStateException("Cannot load node number $prev.");
						}
					} else {
						if ($nextNodeId === FALSE) {
	...
	private function getNode($id)
	{
		...
		if ($magic !== self::INDEX_MAGIC && $magic !== self::DATA_MAGIC) {
			if (!empty($magic)) {
				if (self::$debug) {
					throw new NetteX\InvalidStateException("Node $id has malformed header.");
				}
				$this->deleteNode($id);
			}
			return FALSE;
		...
		if ($node === FALSE) {
			$this->deleteNode($id);
			if (self::$debug) {
				throw new NetteX\InvalidStateException("Cannot deserialize node number $id.");
			}
			return FALSE;
		}
		...
		if (count($node) === 1) {
			...
			if ($nodeInfo[self::TYPE] !== self::DATA) {
				...
				if ($parentId !== -1 && $parentId !== $id) {
					...
					if ($parentNode === FALSE) {
						if (self::$debug) {
							throw new NetteX\InvalidStateException("Cannot load node number $parentId.");
						}
					} else {
				...
				if ($nodeInfo[self::TYPE] === self::PRIORITY) {
					if ($nodeInfo[self::MAX] === -1) {
						if ($nodeInfo[self::PREV_NODE] !== -1) {
							...
							if ($prevNode === FALSE) {
								if (self::$debug) {
									throw new NetteX\InvalidStateException('Cannot load node number ' . $nodeInfo[self::PREV_NODE] . '.');
								}
							} else {
	...
	private function prepareNode($id, $node)
	{
		...
		if ($dataSize > self::NODE_SIZE) {
			if ($isData) {
				throw new NetteX\InvalidStateException('Saving node is bigger than maximum node size.');
			} else {
	...
	private function commitNode($id, $str)
	{
		...
		if ($writen === FALSE) {
			throw new NetteX\InvalidStateException("Cannot write node number $id to journal.");
		}
	}
	...
	private function bisectNode($id, array $node)
	{
		...
		if ($id <= 2) {
			...
			if ($parentNode === FALSE) {
				if (self::$debug) {
					throw new NetteX\InvalidStateException("Cannot load node number $parent.");
				}
			} else {
	...
	private function deleteNode($id)
	{
		...
		if ($end <= (self::HEADER_SIZE + self::NODE_SIZE * ($id + 1))) {
			...
			if (!ftruncate($this->handle, self::HEADER_SIZE + self::NODE_SIZE * ($id + 1))) {
				throw new NetteX\InvalidStateException('Cannot truncate journal file.');
			}
		} else {
			fseek($this->handle, self::HEADER_SIZE + self::NODE_SIZE * $id);
			$writen = fwrite($this->handle, pack('N', 0));
			if ($writen !== self::INT32_SIZE) {
				throw new NetteX\InvalidStateException("Cannot delete node number $id from journal.");
			}
		}
	}
	...
	{
		if (!$this->handle) {
			throw new NetteX\InvalidStateException('File journal file is not opened');
		}

		if (!flock($this->handle, LOCK_EX)) {
			throw new NetteX\InvalidStateException('Cannot acquire exclusive lock on journal.');
		}
...
class FileStorage extends NetteX\Object implements NetteX\Caching\IStorage
{
	...
	function __construct($dir, IJournal $journal = NULL)
	{
		...
		if ($this->dir === FALSE) {
			throw new NetteX\DirectoryNotFoundException("Directory '$dir' not found.");
		}
	...
	function write($key, $data, array $dp)
	{
		...
		if (isset($dp[Cache::TAGS]) || isset($dp[Cache::PRIORITY])) {
			if (!$this->journal) {
				throw new NetteX\InvalidStateException('CacheJournal has not been provided.');
			}
	...
	function clean(array $conds)
	{
		...
		if ($all || $collector) {
			$now = time();
			foreach (NetteX\Utils\Finder::find('*')->from($this->dir)->childFirst() as $entry) {
				$path = (string) $entry;
				if ($entry->isDir()) {
					@rmdir($path);
					continue;
...
class MemcachedStorage extends NetteX\Object implements NetteX\Caching\IStorage
{
	...
	{
		if (!self::isAvailable()) {
			throw new NetteX\NotSupportedException("PHP extension 'memcache' is not loaded.");
		}

		$this->prefix = $prefix;
		$this->journal = $journal;
		$this->memcache = new \Memcache;
		NetteX\Diagnostics\Debugger::tryError();
		$this->memcache->connect($host, $port);
		if (NetteX\Diagnostics\Debugger::catchError($e)) {
			throw new NetteX\InvalidStateException('Memcache::connect(): ' . $e->getMessage(), 0, $e);
		}
	}
	...
	{
		if (isset($dp[Cache::ITEMS])) {
			throw new NetteX\NotSupportedException('Dependent items are not supported by MemcachedStorage.');
		}
...
class MemoryStorage extends NetteX\Object implements NetteX\Caching\IStorage
{
	...
	function clean(array $conds)
	{
		if (!empty($conds[NetteX\Caching\Cache::ALL])) {
			$this->data = array();
		}
	}
...
class PhpFileStorage extends FileStorage
{
	...
	protected function getCacheFile($key)
	{
		return parent::getCacheFile(substr_replace(
			...
			strpos($key, NetteX\Caching\Cache::NAMESPACE_SEPARATOR) + 1,
			0
		)) . '.php';
	}
...
final class Callback extends Object
{
	...
	{
		if (is_array($this->cb)) {
			return new NetteX\Reflection\Method($this->cb[0], $this->cb[1]);
		} else {
			return new NetteX\Reflection\GlobalFunction($this->cb);
		}
	}
...
use NetteX\Caching\Cache;use NetteX\DI;
...
class Configurator extends Object
{
	...
	function __construct($containerClass = 'NetteX\DI\Container')
	{
		self::$instance = $this;
	...
	function loadConfig($file, $section = NULL)
	{
		...
		$cache = new Cache($container->templateCacheStorage, 'NetteX.Configurator');
		...
		$config = NetteX\Config\Config::fromFile($file, $section);
		$code = "<?php\n// source file $file\n\n";
		...
		while (!empty($config['variables'])) {
			...
			foreach ($config['variables'] as $key => $value) {
				...
				} catch (NetteX\InvalidArgumentException $e) {}
			}
		...
		if (isset($config['services'])) {
			foreach ($config['services'] as $key => & $def) {
				if (preg_match('#^NetteX\\\\.*\\\\I?([a-zA-Z]+)$#', strtr($key, '-', '\\'), $m)) {
		...
		foreach ($config as $key => $value) {
			$code .= $this->generateCode('$container->params[?] = ' . (is_array($value) ? 'NetteX\ArrayHash::from(?)' : '?'), $key, $value);
		}
		...
		NetteX\Utils\LimitedScope::evaluate($code, array('container' => $container));
	}
	...
	{
		if (!is_scalar($value)) {
			throw new NetteX\InvalidStateException("Configuration value for directive '$name' is not scalar.");
		}
		...
		if (function_exists('ini_set')) {
			...
			throw new NetteX\NotSupportedException('Required function ini_set() is disabled.');
		}
	}
	...
	static function createServiceApplication(DI\Container $container, array $options = NULL)
	{
		...
		NetteX\Application\UI\Presenter::$invalidLinkMode = $container->params['productionMode']
			? NetteX\Application\UI\Presenter::INVALID_LINK_SILENT
			: NetteX\Application\UI\Presenter::INVALID_LINK_WARNING;

		$class = isset($options['class']) ? $options['class'] : 'NetteX\Application\Application';
	...
	{
		return new NetteX\Application\PresenterFactory(
	...
	{
		return new NetteX\Application\Routers\RouteList;
	}

	static function createServiceHttpRequest()
	{
		$factory = new NetteX\Http\RequestFactory;
	...
	{
		$response = new NetteX\Http\Response;
		if (!$response->isSent()) {
	...
	{
		return new NetteX\Http\Context($container->httpRequest, $container->httpResponse);
	}

	static function createServiceSession(DI\Container $container, array $options = NULL)
	{
		$session = new NetteX\Http\Session($container->httpRequest, $container->httpResponse);
	...
	static function createServiceUser(DI\Container $container)
	{
		...
		$context->addService('session', $container->session);
		return new NetteX\Http\User($context);
	}

	static function createServiceCacheStorage(DI\Container $container)
	{
		if (!isset($container->params['tempDir'])) {
			throw new NetteX\InvalidStateException("Service cacheStorage requires that parameter 'tempDir' contains path to temporary directory.");
		}
		$dir = $container->expand('%tempDir%/cache');
		umask(0000);
		@mkdir($dir, 0777);
		return new NetteX\Caching\Storages\FileStorage($dir, $container->cacheJournal);
	}

	static function createServiceTemplateCacheStorage(DI\Container $container)
	{
		if (!isset($container->params['tempDir'])) {
			throw new NetteX\InvalidStateException("Service templateCacheStorage requires that parameter 'tempDir' contains path to temporary directory.");
		}
		$dir = $container->expand('%tempDir%/cache');
		umask(0000);
		@mkdir($dir, 0777);
		return new NetteX\Caching\Storages\PhpFileStorage($dir);
	}

	static function createServiceCacheJournal(DI\Container $container)
	{
		return new NetteX\Caching\Storages\FileJournal($container->params['tempDir']);
	}

	static function createServiceMailer(DI\Container $container, array $options = NULL)
	{
		if (empty($options['smtp'])) {
			return new NetteX\Mail\SendmailMailer;
		} else {
			return new NetteX\Mail\SmtpMailer($options);
		}
	}

	static function createServiceRobotLoader(DI\Container $container, array $options = NULL)
	{
		$loader = new NetteX\Loaders\RobotLoader;
	...
	static function preloadEnvironment(DI\Container $container)
	{
		...
		if (!@mkdir("$dir/$uniq", 0777)) {
			throw new NetteX\InvalidStateException("Unable to write to directory '$dir'. Make this directory writable.");
		}
		...
		$code .= self::generateCode('NetteX\Caching\Storages\FileStorage::$useDirectories = ?', $useDirs);
		return $code;
	}
...
final class Environment
{
	...
	static function __callStatic($name, $args)
	{
		if (!$args && strncasecmp($name, 'get', 3) === 0) {
			...
			throw new MemberAccessException("Call to undefined static method NetteX\\Environment::$name().");
		}
	}
...
final class Framework
{
	...
	const NAME = 'NetteX Framework',
		VERSION = '2.0-beta',
		REVISION = '648b258 released on 2011-06-13';
		...
		if ($class->hasEventProperty($name)) {
			if (is_array($list = $_this->$name) || $list instanceof \Traversable) {
				foreach ($list as $handler) {
					callbackX($handler)->invokeArgs($args);
				}
			}
			return NULL;
		}
...
class Config
{
	...
	private static $extensions = array(
		'ini' => 'NetteX\Config\IniAdapter',
		'neon' => 'NetteX\Config\NeonAdapter',
	);
	...
	{
		if (!class_exists($class)) {
			throw new NetteX\InvalidArgumentException("Class '$class' was not found.");
		}

		if (!NetteX\Reflection\ClassType::from($class)->implementsInterface('NetteX\Config\IAdapter')) {
			throw new NetteX\InvalidArgumentException("Configuration adapter '$class' is not NetteX\\Config\\IAdapter implementor.");
		}
	...
	static function fromFile($file, $section = NULL)
	{
		...
		if (!isset(self::$extensions[$extension])) {
			throw new NetteX\InvalidArgumentException("Unknown file extension '$file'.");
		}

		$data = call_user_func(array(self::$extensions[$extension], 'load'), $file, $section);
		if ($section) {
			if (!isset($data[$section]) || !is_array($data[$section])) {
				throw new NetteX\InvalidStateException("There is not section [$section] in file '$file'.");
			}
			$data = $data[$section];
		}
		return $data;
...
final class IniAdapter implements IAdapter
{
	...
	{
		if (!is_file($file) || !is_readable($file)) {
			throw new NetteX\FileNotFoundException("File '$file' is missing or is not readable.");
		}

		NetteX\Diagnostics\Debugger::tryError();
		$ini = parse_ini_file($file, TRUE);
		if (NetteX\Diagnostics\Debugger::catchError($e)) {
			throw new NetteX\InvalidStateException('parse_ini_file(): ' . $e->getMessage(), 0, $e);
		}
		...
		foreach ($ini as $secName => $secData) {
			...
			if (is_array($secData)) {
					...
					foreach ($secData as $key => $val) {
						...
						foreach (explode(self::$keySeparator, $key) as $part) {
							if (!isset($cursor[$part]) || is_array($cursor[$part])) {
								...
								throw new NetteX\InvalidStateException("Invalid key '$key' in section [$secName] in file '$file'.");
							}
						}
						$cursor = $val;
					}
				...
				if (count($parts) > 1) {
					...
					if (!isset($data[$parent]) || !is_array($data[$parent])) {
						throw new NetteX\InvalidStateException("Missing parent section [$parent] in file '$file'.");
					}
					$secData = NetteX\Utils\Arrays::mergeTree($secData, $data[$parent]);
					$secName = trim($parts[0]);
					if ($secName === '') {
						throw new NetteX\InvalidStateException("Invalid empty section name in file '$file'.");
					}
				}
			}
			...
			if (self::$keySeparator) {
				...
				foreach (explode(self::$keySeparator, $secName) as $part) {
					if (!isset($cursor[$part]) || is_array($cursor[$part])) {
						...
						throw new NetteX\InvalidStateException("Invalid section [$secName] in file '$file'.");
					}
				}
			} else {
				$cursor = & $data[$secName];
			...
			if (is_array($secData) && is_array($cursor)) {
				$secData = NetteX\Utils\Arrays::mergeTree($secData, $cursor);
			}
	...
	{
		$output = array();
		$output[] = '; generated by NetteX';
		$output[] = '';

		foreach ($config as $secName => $secData) {
			if (!(is_array($secData) || $secData instanceof \Traversable)) {
				throw new NetteX\InvalidStateException("Invalid section '$secName'.");
			}
		...
		if (!file_put_contents($file, implode(PHP_EOL, $output))) {
			throw new NetteX\IOException("Cannot write file '$file'.");
		}
	}
	...
	private static function build($input, & $output, $prefix)
	{
		foreach ($input as $key => $val) {
			if (is_array($val) || $val instanceof \Traversable) {
				...
				throw new NetteX\InvalidArgumentException("The '$prefix$key' item must be scalar or array, " . gettype($val) ." given.");
			}
		}
	}
...
final class NeonAdapter implements IAdapter
{
	...
	static function save($config, $file)
	{
		if (!file_put_contents($file, "# generated by NetteX\n\n" . Neon::encode($config, Neon::BLOCK))) {
			throw new NetteX\IOException("Cannot write file '$file'.");
		}
	}
...
use NetteX;use NetteX\ObjectMixin;use PDO;
...
if (class_exists('PDO')){ class Connection extends PDO
{
	...
	function __construct($dsn, $username = NULL, $password  = NULL, array $options = NULL)
	{
		...
		$this->setAttribute(PDO::ATTR_STATEMENT_CLASS, array('NetteX\Database\Statement', array($this)));

		$class = 'NetteX\Database\Drivers\\' . $this->getAttribute(PDO::ATTR_DRIVER_NAME) . 'Driver';
		if (class_exists($class)) {
	...
	function loadFile($file)
	{
		...
		$handle = @fopen($file, 'r');
		if (!$handle) {
			throw new NetteX\FileNotFoundException("Cannot open file '$file'.");
		}
	...
	static function getReflection()
	{
		return new NetteX\Reflection\ClassType(get_called_class());
	}
...
namespace NetteX\Database\Diagnostics {

use NetteX;use NetteX\Database\Connection;use NetteX\Diagnostics\Debugger;

class ConnectionPanel extends NetteX\Object implements NetteX\Diagnostics\IBarPanel
{
	...
	static function initialize(Connection $connection)
	{
		if (!Debugger::$productionMode) {
			$panel = new static;
			$connection->onQuery[] = callbackX($panel, 'logQuery');
			Debugger::$bar->addPanel($panel);
			Debugger::$blueScreen->addPanel(callbackX($panel, 'renderException'), __CLASS__);
		}
	}

	function logQuery(NetteX\Database\Statement $result, array $params = NULL)
	{
		if ($this->disabled) {
			return;
		}
		...
		foreach (debug_backtrace(FALSE) as $row) {
			if (isset($row['file']) && is_file($row['file']) && strpos($row['file'], NETTEX_DIR . DIRECTORY_SEPARATOR) !== 0) {
	...
	function getTab()
	{
		return '<span title="NetteX\\Database ' . htmlSpecialChars($this->name) . '">'
	...
	function getPanel()
	{
		...
		foreach ($this->queries as $i => $query) {
			...
			$s .= '</td><td class="nette-DbConnectionPanel-sql">' . Connection::highlightSql(NetteX\Utils\Strings::truncate($sql, self::$maxLength));
			if ($explain) {
			...
			if ($source) {
				$s .= NetteX\Diagnostics\Helpers::editorLink($source[0], $source[1])->class('nette-DbConnectionPanel-source');
			}
...
namespace NetteX\Database\Drivers {

use NetteX;

class MsSqlDriver extends NetteX\Object implements NetteX\Database\ISupplementalDriver
{
	...
	function __construct(NetteX\Database\Connection $connection, array $options)
	{
	...
	function applyLimit(&$sql, $limit, $offset)
	{
		...
		if ($offset) {
			throw new NetteX\NotImplementedException('Offset is not implemented.');
		}
	}
...
class MySqlDriver extends NetteX\Object implements NetteX\Database\ISupplementalDriver
{
...
class OciDriver extends NetteX\Object implements NetteX\Database\ISupplementalDriver
{
...
class OdbcDriver extends NetteX\Object implements NetteX\Database\ISupplementalDriver
{
	...
	function applyLimit(&$sql, $limit, $offset)
	{
		...
		if ($offset) {
			throw new NetteX\InvalidArgumentException('Offset is not implemented in driver odbc.');
		}
	}
...
class PgSqlDriver extends NetteX\Object implements NetteX\Database\ISupplementalDriver
{
...
class SqliteDriver extends NetteX\Object implements NetteX\Database\ISupplementalDriver
{
...
class Sqlite2Driver extends SqliteDriver
{
	...
	function formatLike($value, $pos)
	{
		throw new NetteX\NotSupportedException;
	}
...
namespace NetteX\Database\Reflection {

use NetteX;

class DatabaseReflection extends NetteX\Object
{
...
class Row extends NetteX\ArrayHash
{
...
class SqlPreprocessor extends NetteX\Object
{
	...
	function process($sql, $params)
	{
		...
		$sql = NetteX\Utils\Strings::replace($sql, '~\'.*?\'|".*?"|:[a-zA-Z0-9_]+:|\?~s', array($this, 'callback'));
...
use PDO;use NetteX\ObjectMixin;
...
if (class_exists('PDO')){ class Statement extends \PDOStatement
{
	...
	{
		$this->connection = $connection;
		$this->setFetchMode(PDO::FETCH_CLASS, 'NetteX\Database\Row', array($this));
	}
...
namespace NetteX\Database\Table {

use NetteX;

class ActiveRow extends NetteX\Object implements \IteratorAggregate, \ArrayAccess
{
...
class Selection extends NetteX\Object implements \Iterator, \ArrayAccess, \Countable
{
	...
	function __construct($table, NetteX\Database\Connection $connection)
	{
		$this->name = $table;
		$this->connection = $connection;
	...
	function update($data)
	{
		if ($data instanceof \Traversable) {
			...
			throw new NetteX\InvalidArgumentException;
		}
...
class Container extends NetteX\FreezableObject implements IContainer
{
	...
	function addService($name, $service, $tags = NULL)
	{
		...
		if (!is_string($name) || $name === '') {
			throw new NetteX\InvalidArgumentException("Service name must be a non-empty string, " . gettype($name) . " given.");
		}

		if (isset($this->registry[$name]) || method_exists($this, "createService$name")) {
			throw new NetteX\InvalidStateException("Service '$name' has already been registered.");
		}
		...
		} elseif (is_object($service) && !$service instanceof \Closure && !$service instanceof NetteX\Callback) {
		...
		$this->factories[$name] = array(callbackX($factory));
		$this->tags[$name] = $tags;
	...
	function getService($name)
	{
		if (isset($this->registry[$name])) {
			...
			throw new NetteX\InvalidStateException("Circular reference detected for services: "
		...
		if (isset($this->factories[$name])) {
			...
			if (!$factory->isCallable()) {
				throw new NetteX\InvalidStateException("Unable to create service '$name', factory '$factory' is not callable.");
			}
		...
		if (isset($e)) {
			...
			throw new NetteX\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not object.");

		} elseif (isset($this->tags[$name][self::TAG_TYPEHINT][0]) && !$service instanceof $this->tags[$name][self::TAG_TYPEHINT][0]) {
			throw new NetteX\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not '{$this->tags[$name][self::TAG_TYPEHINT][0]}' type.");
		}
	...
	function expand($s)
	{
		if (is_string($s) && strpos($s, '%') !== FALSE) {
			...
			return @preg_replace_callback('#%([a-z0-9._-]*)%#i', function ($m) use($that) {
				...
				if ($param === '') {
					return '%';
				} elseif (!is_scalar($val = NetteX\Utils\Arrays::get((array) $that->params, explode('.', $param)))) {
					throw new NetteX\InvalidStateException("Parameter '$param' is not scalar.");
				}
				return $val;
			}, $s);
		}
	...
	function __set($name, $service)
	{
		...
		if (!is_string($name) || $name === '') {
			...
			throw new NetteX\InvalidArgumentException("Service must be a object, " . gettype($service) . " given.");
		}
		$this->registry[$name] = $service;
	}
...
class ContainerBuilder extends NetteX\Object
{
			...
			if (isset($definition['class'])) {
				...
				$factory = function($container) use($class, $arguments, $methods, $expander) {
					...
					if ($arguments) {
						...
						$service = NetteX\Reflection\ClassType::from($class)->newInstanceArgs($arguments);
					} else {
						$service = new $class;
					}
				...
				throw new NetteX\InvalidStateException("The definition of service '$name' is missing factory method.");
			}
...
class MissingServiceException extends NetteX\InvalidStateException
{
}

class AmbiguousServiceException extends NetteX\InvalidStateException
{
}

class ServiceBuilder extends NetteX\Object implements IServiceBuilder
{
	...
	function createService(NetteX\DI\IContainer $container)
	{
		if (!class_exists($this->class)) {
			throw new NetteX\InvalidStateException("Cannot instantiate service, class '$this->class' not found.");
		}
...
final class Helpers
{
	...
	static function editorLink($file, $line)
	{
		if (Debugger::$editor && is_file($file)) {
			...
			return NetteX\Utils\Html::el('span')->setText($file);
		}
	}
...
class ControlGroup extends NetteX\Object
{
	...
	function add()
	{
		foreach (func_get_args() as $num => $item) {
			if ($item instanceof IControl) {
				...
				throw new NetteX\InvalidArgumentException("Only IFormControl items are allowed, the #$num parameter is invalid.");
			}
		}
		return $this;
	}
...
namespace NetteX\Forms\Controls {

use NetteX;use NetteX\Forms\IControl;use NetteX\Utils\Html;use NetteX\Forms\Form;use NetteX\Forms\Rule;

abstract class BaseControl extends NetteX\ComponentModel\Component implements IControl
{
	...
	{
		$this->monitor('NetteX\Forms\Form');
		parent::__construct();
		$this->control = Html::el('input');
		$this->label = Html::el('label');
		$this->caption = $caption;
		$this->rules = new NetteX\Forms\Rules($this);
	}
	...
	function getHtmlName()
	{
		if ($this->htmlName === NULL) {
			$name = str_replace(self::NAME_SEPARATOR, '][', $this->lookupPath('NetteX\Forms\Form'), $count);
			if ($count) {
	...
	function getHtmlId()
	{
		if ($this->htmlId === FALSE) {
			...
			$this->htmlId = sprintf(self::$idMask, $this->getForm()->getName(), $this->lookupPath('NetteX\Forms\Form'));
		}
		return $this->htmlId;
	}
	...
	function loadHttpData()
	{
		...
		$this->setValue(NetteX\Utils\Arrays::get($this->getForm()->getHttpData(), $path, NULL));
	}
...
class SubmitButton extends Button implements NetteX\Forms\ISubmitterControl
{
	...
	static function validateSubmitted(NetteX\Forms\ISubmitterControl $control)
	{
...
class ImageButton extends SubmitButton
{
	...
	function loadHttpData()
	{
		...
		$this->setValue(NetteX\Utils\Arrays::get($this->getForm()->getHttpData(), $path, NULL) !== NULL);
	}
...
class SelectBox extends BaseControl
{
	...
	function setItems(array $items, $useKeys = TRUE)
	{
		...
		foreach ($items as $key => $value) {
			...
			foreach ($value as $key2 => $value2) {
				if (!$this->useKeys) {
					if (!is_scalar($value2)) {
						throw new NetteX\InvalidArgumentException("All items must be scalar.");
					}
					$key2 = $value2;
				}

				if (isset($this->allowed[$key2])) {
					throw new NetteX\InvalidArgumentException("Items contain duplication for key '$key2'.");
				}
	...
	function getControl()
	{
		...
		$option = NetteX\Utils\Html::el('option');
		...
		foreach ($this->items as $key => $value) {
			...
			foreach ($value as $key2 => $value2) {
				if ($value2 instanceof NetteX\Utils\Html) {
...
abstract class TextBase extends BaseControl
{
	...
	function getControl()
	{
		...
		foreach ($this->getRules() as $rule) {
			if ($rule->type === NetteX\Forms\Rule::VALIDATOR && !$rule->isNegative
	...
	function addRule($operation, $message = NULL, $arg = NULL)
	{
		if ($operation === Form::FLOAT) {
			$this->addFilter(callbackX(__CLASS__, 'filterFloat'));
		}
...
class TextInput extends TextBase
{
	...
	function __construct($label = NULL, $cols = NULL, $maxLength = NULL)
	{
		...
		$this->filters[] = callbackX($this, 'sanitize');
		$this->value = '';
	}

	function sanitize($value)
	{
		if ($this->control->maxlength && NetteX\Utils\Strings::length($value) > $this->control->maxlength) {
			$value = iconv_substr($value, 0, $this->control->maxlength, 'UTF-8');
		}
		return NetteX\Utils\Strings::trim(strtr($value, "\r\n", '  '));
	}
	...
	function getControl()
	{
		$control = parent::getControl();
		foreach ($this->getRules() as $rule) {
			if ($rule->isNegative || $rule->type !== NetteX\Forms\Rule::VALIDATOR) {

			} elseif ($rule->operation === NetteX\Forms\Form::RANGE && $control->type !== 'text') {
				list($control->min, $control->max) = $rule->arg;

			} elseif ($rule->operation === NetteX\Forms\Form::PATTERN) {
...
class UploadControl extends BaseControl
{
	...
	protected function attached($form)
	{
		if ($form instanceof NetteX\Forms\Form) {
			if ($form->getMethod() !== NetteX\Forms\Form::POST) {
				throw new NetteX\InvalidStateException('File upload requires method POST.');
			}
...
namespace NetteX\Forms\Rendering {

use NetteX;use NetteX\Utils\Html;

class DefaultFormRenderer extends NetteX\Object implements NetteX\Forms\IFormRenderer
{
	...
	function render(NetteX\Forms\Form $form, $mode = NULL)
	{
		if ($this->form !== $form) {
			$this->form = $form;
			$this->init();
	...
	function renderEnd()
	{
		$s = '';
		foreach ($this->form->getControls() as $control) {
			if ($control instanceof NetteX\Forms\Controls\HiddenField && !$control->getOption('rendered')) {
	...
	function renderErrors(NetteX\Forms\IControl $control = NULL)
	{
	...
	{
		if (!($parent instanceof NetteX\Forms\Container || $parent instanceof NetteX\Forms\ControlGroup)) {
			throw new NetteX\InvalidArgumentException("Argument must be FormContainer or FormGroup instance.");
		}
		...
		foreach ($parent->getControls() as $control) {
			if ($control->getOption('rendered') || $control instanceof NetteX\Forms\Controls\HiddenField || $control->getForm(FALSE) !== $this->form) {

			} elseif ($control instanceof NetteX\Forms\Controls\Button) {
				$buttons[] = $control;
	...
	function renderPair(NetteX\Forms\IControl $control)
	{
	...
	function renderPairMulti(array $controls)
	{
		...
		foreach ($controls as $control) {
			if (!$control instanceof NetteX\Forms\IControl) {
				throw new NetteX\InvalidArgumentException("Argument must be array of IFormControl instances.");
			}
			$s[] = (string) $control->getControl();
		}
		$pair = $this->getWrapper('pair container');
	...
	function renderLabel(NetteX\Forms\IControl $control)
	{
		$head = $this->getWrapper('label container');

		if ($control instanceof NetteX\Forms\Controls\Checkbox || $control instanceof NetteX\Forms\Controls\Button) {
	...
	function renderControl(NetteX\Forms\IControl $control)
	{
...
final class Rule extends NetteX\Object
{
...
final class Rules extends NetteX\Object implements \IteratorAggregate
{
	...
	private function adjustOperation($rule)
	{
		...
		if (!$this->getCallback($rule)->isCallable()) {
			...
			throw new NetteX\InvalidArgumentException("Unknown operation$operation for control '{$rule->control->name}'.");
		}
	}
	...
	private function getCallback($rule)
	{
		...
		if (is_string($op) && strncmp($op, ':', 1) === 0) {
			return callbackX(get_class($rule->control), self::VALIDATE_PREFIX . ltrim($op, ':'));
		} else {
			return callbackX($op);
		}
	}
...
class Context extends NetteX\Object
{
...
class FileUpload extends NetteX\Object
{
	...
	function getContentType()
	{
		if ($this->isOk() && $this->type === NULL) {
			$this->type = NetteX\Utils\MimeTypeDetector::fromFile($this->tmpName);
		}
		return $this->type;
	}
	...
	function move($dest)
	{
		...
		if (!$func($this->tmpName, $dest)) {
			throw new NetteX\InvalidStateException("Unable to move uploaded file '$this->tmpName' to '$dest'.");
		}
		chmod($dest, 0644);
		$this->tmpName = $dest;
		return $this;
	...
	function toImage()
	{
		return NetteX\Image::fromFile($this->tmpName);
	}
...
class Request extends NetteX\Object implements IRequest
{
	...
	{
		$args = func_get_args();
		return NetteX\Utils\Arrays::get($this->files, $args, NULL);
	}
...
class RequestFactory extends NetteX\Object
{
...
final class Response extends NetteX\Object implements IResponse
{
	...
	function setCode($code)
	{
		...
		if (!isset($allowed[$code])) {
			throw new NetteX\InvalidArgumentException("Bad HTTP response '$code'.");

		} elseif (headers_sent($file, $line)) {
			throw new NetteX\InvalidStateException("Cannot set HTTP code after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
	...
	function setHeader($name, $value)
	{
		if (headers_sent($file, $line)) {
			throw new NetteX\InvalidStateException("Cannot send header after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
		}
	...
	function setExpiration($time)
	{
		...
		$time = NetteX\DateTime::from($time);
	...
	function __destruct()
	{
		if (self::$fixIE && isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE ') !== FALSE
			...
			echo NetteX\Utils\Strings::random(2e3, " \t\r\n");
			self::$fixIE = FALSE;
		}
	}
	...
	{
		if (headers_sent($file, $line)) {
			throw new NetteX\InvalidStateException("Cannot set cookie after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
		}

		setcookie(
			$name,
			$value,
			$time ? NetteX\DateTime::from($time)->format('U') : 0,
	...
	{
		if (headers_sent($file, $line)) {
			throw new NetteX\InvalidStateException("Cannot delete cookie after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
		}
...
class Session extends NetteX\Object
{
	...
	function start()
	{
		if (self::$started) {
			...
			throw new NetteX\InvalidStateException('A session had already been started by session.auto_start or session_start().');
		}
		...
		if (NetteX\Diagnostics\Debugger::catchError($e)) {
			@session_write_close();
			throw new NetteX\InvalidStateException('session_start(): ' . $e->getMessage(), 0, $e);
		}
		...
		if (!$browserKey) {
			$browserKey = NetteX\Utils\Strings::random();
		}
					...
					foreach ($metadata as $variable => $value) {
						if ((!empty($value['B']) && $browserClosed) || (!empty($value['T']) && $now > $value['T'])
							|| ($variable !== '' && is_object($nf['DATA'][$section][$variable]) && (isset($value['V']) ? $value['V'] : NULL)
								!== NetteX\Reflection\ClassType::from($nf['DATA'][$section][$variable])->getAnnotation('serializationVersion'))
						) {
							if ($variable === '') {
	...
	{
		if (!self::$started) {
			throw new NetteX\InvalidStateException('Session is not started.');
		}
	...
	function regenerateId()
	{
		if (self::$started) {
			if (headers_sent($file, $line)) {
				throw new NetteX\InvalidStateException("Cannot regenerate session ID after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
			}
			session_regenerate_id(TRUE);
	...
	function setName($name)
	{
		if (!is_string($name) || !preg_match('#[^0-9.][^.]*$#A', $name)) {
			throw new NetteX\InvalidArgumentException('Session name must be a string and cannot contain dot.');
		}
	...
	function getSection($section, $class = 'NetteX\Http\SessionSection')
	{
	...
	private function configure(array $config)
	{
		...
		foreach ($config as $key => $value) {
			...
			if ($value === NULL) {
				...
				if (self::$started) {
					throw new NetteX\InvalidStateException("Unable to set '$key' when session has been started.");
				}
				$key = "session_$key";
				$key($value);
				...
				if (ini_get($key) != $value && !NetteX\Framework::$iAmUsingBadHost) {
					throw new NetteX\NotSupportedException('Required function ini_set() is disabled.');
				}
	...
	function setExpiration($time)
	{
		if (empty($time)) {
			...
			$time = NetteX\DateTime::from($time)->format('U') - time();
			return $this->setOptions(array(
				'gc_maxlifetime' => $time,
	...
	{
		if (self::$started) {
			throw new NetteX\InvalidStateException("Unable to set storage when session has been started.");
		}
...
final class SessionSection extends NetteX\Object implements \IteratorAggregate, \ArrayAccess
{
	...
	{
		if (!is_string($name)) {
			throw new NetteX\InvalidArgumentException("Session namespace must be a string, " . gettype($name) ." given.");
		}
	...
	function __set($name, $value)
	{
		...
		if (is_object($value)) {
			$this->meta[$name]['V'] = NetteX\Reflection\ClassType::from($value)->getAnnotation('serializationVersion');
		}
	}
	...
	function setExpiration($time, $variables = NULL)
	{
		...
		if (empty($time)) {
			...
			$time = NetteX\DateTime::from($time)->format('U');
...
class Url extends NetteX\FreezableObject
{
	...
	function __construct($url = NULL)
	{
		if (is_string($url)) {
			$parts = @parse_url($url);
			if ($parts === FALSE) {
				throw new NetteX\InvalidArgumentException("Malformed or unsupported URI '$url'.");
			}
...
use NetteX\Security\IAuthenticator;use NetteX\Security\IAuthorizator;use NetteX\Security\IIdentity;

class User extends NetteX\Object implements IUser
{
	...
	protected function getSessionSection($need)
	{
		...
		$this->session = $session = $this->context->session->getSection('NetteX.Web.User/' . $this->namespace);
...
class CachingIterator extends \CachingIterator implements \Countable
{
	...
	function __construct($iterator)
	{
		if (is_array($iterator) || $iterator instanceof \stdClass) {
			...
			throw new NetteX\InvalidArgumentException("Invalid argument passed to foreach resp. " . __CLASS__ . "; array or Traversable expected, " . (is_object($iterator) ? get_class($iterator) : gettype($iterator)) ." given.");
		}
	...
	function count()
	{
		...
		if ($inner instanceof \Countable) {
			...
			throw new NetteX\NotSupportedException('Iterator is not countable.');
		}
	}
	...
	function __call($name, $args)
	{
		return NetteX\ObjectMixin::call($this, $name, $args);
	}

	function &__get($name)
	{
		return NetteX\ObjectMixin::get($this, $name);
	}

	function __set($name, $value)
	{
		return NetteX\ObjectMixin::set($this, $name, $value);
	}

	function __isset($name)
	{
		return NetteX\ObjectMixin::has($this, $name);
	}

	function __unset($name)
	{
		NetteX\ObjectMixin::remove($this, $name);
	}
...
class Engine extends NetteX\Object
{
...
class HtmlNode extends NetteX\Object
{
	...
	{
		$this->name = $name;
		$this->isEmpty = isset(NetteX\Utils\Html::$emptyElements[strtolower($this->name)]);
	}

}

class MacroNode extends NetteX\Object
{
...
namespace NetteX\Latte\Macros {

use NetteX;use NetteX\Latte;

class CacheMacro extends NetteX\Object implements Latte\IMacro
{
	...
	{
		if ($this->used) {
			return array('NetteX\Latte\Macros\CacheMacro::initRuntime($template, $_g);');
		}
	}
	...
	function nodeOpened(Latte\MacroNode $node)
	{
		...
		return Latte\PhpWriter::using($node)
			->write('<?php if (NetteX\Latte\Macros\CacheMacro::createCache($netteCacheStorage, %var, $_g->caches, %node.array?)) { ?>',
				NetteX\Utils\Strings::random()
			);
	}
	...
	static function initRuntime($template, $global)
	{
		if (!empty($global->caches)) {
			end($global->caches)->dependencies[NetteX\Caching\Cache::FILES][] = $template->getFile();
		}
	}

	static function createCache(NetteX\Caching\IStorage $cacheStorage, $key, & $parents, $args = NULL)
	{
		if ($args) {
		...
		if ($parents) {
			end($parents)->dependencies[NetteX\Caching\Cache::ITEMS][] = $key;
		}

		$cache = new NetteX\Caching\Cache($cacheStorage, 'NetteX.Templating.Cache');
		if ($helper = $cache->start($key)) {
			$helper->dependencies = array(
				NetteX\Caching\Cache::TAGS => isset($args['tags']) ? $args['tags'] : NULL,
				NetteX\Caching\Cache::EXPIRATION => isset($args['expire']) ? $args['expire'] : '+ 7 days',
			);
			$parents[] = $helper;
		}
		return $helper;
...
use NetteX\Latte\MacroNode;

class MacroSet extends NetteX\Object implements Latte\IMacro
{
	...
	private function compile(MacroNode $node, $def)
	{
		...
		if (is_string($def)) {
			...
			$code = callbackX($def)->invoke($node, $writer);
			if ($code === FALSE) {
				return FALSE;
			}
		}
...
use NetteX\Latte\ParseException;
...
class CoreMacros extends MacroSet
{
	...
	function finalize()
	{
		return array('list($_l, $_g) = NetteX\Latte\Macros\CoreMacros::initRuntime($template, '
	...
	{
		$code = $writer->write('NetteX\Latte\Macros\CoreMacros::includeTemplate(%node.word, %node.array? + $template->getParams(), $_l->templates[%var])',
	...
	{
		return '$iterations = 0; foreach ($iterator = $_l->its[] = new NetteX\Iterators\CachingIterator('
	...
	{
		return $writer->write('echo NetteX\Utils\Html::el(NULL, %node.array)->attributes()');
	}

	function macroOldAttr(MacroNode $node)
	{
		return NetteX\Utils\Strings::replace($node->args . ' ', '#\)\s+#', ')->');
	}

	function macroDump(MacroNode $node, $writer)
	{
		$args = $writer->formatArgs();
		return $writer->write('NetteX\Diagnostics\Debugger::barDump(' . ($node->args ? "array(%var => $args)" : 'get_defined_vars()')
			. ', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))', $args);
	}

	function macroDebugbreak()
	{
	...
	{
		if ($destination instanceof NetteX\Templating\ITemplate) {
			$tpl = $destination;

		} elseif ($destination == NULL) {
			throw new NetteX\InvalidArgumentException("Template file name was not specified.");

		} else {
			$tpl = clone $template;
			if ($template instanceof NetteX\Templating\IFileTemplate) {
...
if (!empty($control->snippetMode)) {
	...
	function macroInclude(MacroNode $node, $writer)
	{
		...
		if (isset($this->namedBlocks[$destination]) && !$parent) {
			...
			$cmd = 'NetteX\Latte\Macros\UIMacros::callBlock' . ($parent ? 'Parent' : '') . "(\$_l, $name, %node.array? + \$template->getParams())";
		}
	...
	{
		return $writer->write('NetteX\Latte\Macros\CoreMacros::includeTemplate(%node.word, %node.array? + get_defined_vars(), $_l->templates[%var])->render()',
			$this->parser->templateId);
	}
	...
	function macroControl(MacroNode $node, $writer)
	{
		...
		return ($name[0] === '$' ? "if (is_object($name)) \$_ctrl = $name; else " : '')
			...
			. 'if ($_ctrl instanceof NetteX\Application\UI\IPartiallyRenderable) $_ctrl->validateControl(); '
			. "\$_ctrl->$method($param)";
	}
	...
	{
		return $writer->write(($node->args ? 'try { $presenter->link(%node.word, %node.array?); } catch (NetteX\Application\UI\InvalidLinkException $e) {}' : '')
	...
	static function callBlock($context, $name, $params)
	{
		if (empty($context->blocks[$name])) {
			throw new NetteX\InvalidStateException("Cannot include undefined block '$name'.");
		}
	...
	static function callBlockParent($context, $name, $params)
	{
		if (empty($context->blocks[$name]) || ($block = next($context->blocks[$name])) === FALSE) {
			throw new NetteX\InvalidStateException("Cannot include undefined parent block '$name'.");
		}
		$block($context, $params);
	}
	...
	static function renderSnippets($control, $local, $params)
	{
		...
		if ($control instanceof NetteX\Application\UI\Control) {
			foreach ($control->getComponents(FALSE, 'NetteX\Application\UI\Control') as $child) {
...
use NetteX;use NetteX\Utils\Strings;

class Tokenizer extends NetteX\Object
{
...
class MacroTokenizer extends NetteX\Utils\Tokenizer
{
...
class FilterException extends NetteX\InvalidStateException
{
...
class ParseException extends NetteX\Templating\FilterException
{
}

use NetteX\Utils\Strings;

class Parser extends NetteX\Object
{
	...
	private function contextTag()
	{
		...
		if (!$matches || !empty($matches['macro']) || !empty($matches['comment'])) {
			...
			if ($isEmpty) {
				$matches[0] = (NetteX\Utils\Html::$xhtml ? ' />' : '>')
...
class PhpWriter extends NetteX\Object
{
	...
	function write($mask)
	{
		...
		$me = $this;
		$mask = NetteX\Utils\Strings::replace($mask, '#%escape\(((?>[^()]+)|\((?1)\))*\)#', function($m) use($me) {
			return $me->escape(substr($m[0], 8, -1));
		});

		return NetteX\Utils\Strings::replace($mask, '#([,+]\s*)?%(node\.word|node\.array|node\.args|modify|var)(\?)?(\s*\+\s*)?()#',
	...
	function escape($s)
	{
		...
		case Parser::CONTEXT_TEXT:
			return "NetteX\\Templating\\DefaultHelpers::escapeHtml($s, ENT_NOQUOTES)";
		case Parser::CONTEXT_TAG:
			return "NetteX\\Templating\\DefaultHelpers::escapeHtml($s)";
		case Parser::CONTEXT_ATTRIBUTE:
			list(, $name, $quote) = $this->context;
			$quote = $quote === '"' ? '' : ', ENT_QUOTES';
			if (strncasecmp($name, 'on', 2) === 0) {
				return "htmlSpecialChars(NetteX\\Templating\\DefaultHelpers::escapeJs($s)$quote)";
			} elseif ($name === 'style') {
				return "htmlSpecialChars(NetteX\\Templating\\DefaultHelpers::escapeCss($s)$quote)";
			} else {
				return "htmlSpecialChars($s$quote)";
			}
		case Parser::CONTEXT_COMMENT:
			return "NetteX\\Templating\\DefaultHelpers::escapeHtmlComment($s)";
		case Parser::CONTEXT_CDATA;
			return 'NetteX\Templating\DefaultHelpers::escape' . ucfirst($this->context[1]) . "($s)";
...
use NetteX;use NetteX\Utils\Strings;use NetteX\Caching\Cache;
...
class RobotLoader extends AutoLoader
{
	...
	function __construct()
	{
		if (!extension_loaded('tokenizer')) {
			throw new NetteX\NotSupportedException("PHP extension Tokenizer is not loaded.");
		}
	}
	...
	function register()
	{
		...
		if (isset($this->list[strtolower(__CLASS__)]) && class_exists('NetteX\Loaders\NetteLoader', FALSE)) {
	...
	function tryLoad($type)
	{
			...
			if ($initiator === 'class_exists' || $initiator === 'interface_exists') {
				...
				if ($this->autoRebuild && $this->rebuilt) {
					$this->getCache()->save($this->getKey(), $this->list, array(
						Cache::CONSTS => 'NetteX\Framework::REVISION',
					));
				}
			}
		...
		if (isset($this->list[$type][0])) {
			NetteX\Utils\LimitedScope::load($this->list[$type][0]);
			self::$count++;
		}
	}
	...
	function rebuild()
	{
		$this->getCache()->save($this->getKey(), callbackX($this, '_rebuildCallback'), array(
			Cache::CONSTS => 'NetteX\Framework::REVISION',
		));
		$this->rebuilt = TRUE;
	}
	...
	function addDirectory($path)
	{
		foreach ((array) $path as $val) {
			$real = realpath($val);
			if ($real === FALSE) {
				throw new NetteX\DirectoryNotFoundException("Directory '$val' not found.");
			}
			$this->scanDirs[] = $real;
		}
		return $this;
	...
	private function addClass($class, $file, $time)
	{
		...
		if (isset($this->list[$lClass][0]) && ($file2 = $this->list[$lClass][0]) !== $file && is_file($file2)) {
			...
			$e = new NetteX\InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in " . $this->list[$lClass][0] . ".");
			{
				throw $e;
			}
		}
	...
	private function scanDirectory($dir)
	{
		if (is_dir($dir)) {
			...
			$iterator = NetteX\Utils\Finder::findFiles(is_array($this->acceptFiles) ? $this->acceptFiles : Strings::split($this->acceptFiles, '#[,\s]+#'))
	...
	function setCacheStorage(NetteX\Caching\IStorage $storage)
	{
	...
	protected function getCache()
	{
		if (!$this->cacheStorage) {
			...
			$this->cacheStorage = new NetteX\Caching\Storages\DevNullStorage;
		}
		return new Cache($this->cacheStorage, 'NetteX.RobotLoader');
	}
...
class MimePart extends NetteX\Object
{
	...
	function setHeader($name, $value, $append = FALSE)
	{
		if (!$name || preg_match('#[^a-z0-9-]#i', $name)) {
			throw new NetteX\InvalidArgumentException("Header name must be non-empty alphanumeric string, '$name' given.");
		}
		...
		if ($value == NULL) {
			...
			foreach ($value as $email => $name) {
				if ($name !== NULL && !Strings::checkEncoding($name)) {
					throw new NetteX\InvalidArgumentException("Name is not valid UTF-8 string.");
				}

				if (!preg_match('#^[^@",\s]+@[^@",\s]+\.[a-z]{2,10}$#i', $email)) {
					throw new NetteX\InvalidArgumentException("Email address '$email' is not valid.");
				}

				if (preg_match('#[\r\n]#', $name)) {
					throw new NetteX\InvalidArgumentException("Name must not contain line separator.");
				}
				$tmp[$email] = $name;
			}
			...
			if (!Strings::checkEncoding($value)) {
				throw new NetteX\InvalidArgumentException("Header is not valid UTF-8 string.");
			}
	...
	function generateMessage()
	{
		...
		if ($body !== '') {
			...
			default:
				throw new NetteX\InvalidStateException('Unknown encoding.');
			}
		}
...
class Message extends MimePart
{
	...
	public static $defaultMailer = 'NetteX\Mail\SendmailMailer';
	...
	private function createAttachment($file, $content, $contentType, $disposition)
	{
		...
		if ($content === NULL) {
			...
			if ($content === FALSE) {
				throw new NetteX\FileNotFoundException("Unable to read file '$file'.");
			}
		} else {
			$content = (string) $content;
		}
		$part->setBody($content);
		$part->setContentType($contentType ? $contentType : NetteX\Utils\MimeTypeDetector::fromString($content));
	...
	{
		if ($this->html instanceof NetteX\Templating\ITemplate) {
			$this->html->mail = $this;
			if ($this->basePath === NULL && $this->html instanceof NetteX\Templating\IFileTemplate) {
	...
	{
		$text = $this->getBody();
		if ($text instanceof NetteX\Templating\ITemplate) {
			$text->mail = $this;
...
class SendmailMailer extends NetteX\Object implements IMailer
{
	...
	function send(Message $mail)
	{
		...
		if (NetteX\Diagnostics\Debugger::catchError($e)) {
			throw new NetteX\InvalidStateException('mail(): ' . $e->getMessage(), 0, $e);

		} elseif (!$res) {
			throw new NetteX\InvalidStateException('Unable to send email.');
		}
	}

}

class SmtpMailer extends NetteX\Object implements IMailer
{
...
class Annotation extends NetteX\Object implements IAnnotation
{
...
final class AnnotationsParser
{
	...
	static function getAll(\Reflector $r)
	{
		...
		if (self::$useReflection) {
			...
			if (!self::$cacheStorage) {
				...
				self::$cacheStorage = new NetteX\Caching\Storages\DevNullStorage;
			}
			$outerCache = new NetteX\Caching\Cache(self::$cacheStorage, 'NetteX.Reflection.Annotations');
	...
	static function setCacheStorage(NetteX\Caching\IStorage $storage)
	{
...
class GlobalFunction extends \ReflectionFunction
{
	...
	function toCallback()
	{
		return new NetteX\Callback($this->value);
	}
...
class Method extends \ReflectionMethod
{
	...
	function toCallback()
	{
		return new NetteX\Callback(parent::getDeclaringClass()->getName(), $this->getName());
	}
...
class Parameter extends \ReflectionParameter
{
	...
	{
		return ($tmp = NetteX\Utils\Strings::match($this, '#>\s+([a-z0-9_\\\\]+)#i')) ? $tmp[1] : NULL;
	}
...
class Identity extends NetteX\FreezableObject implements IIdentity
{
...
class Permission extends NetteX\Object implements IAuthorizator
{
	...
	function addRole($role, $parents = NULL)
	{
		...
		if (isset($this->roles[$role])) {
			throw new NetteX\InvalidStateException("Role '$role' already exists in the list.");
		}
	...
	private function checkRole($role, $need = TRUE)
	{
		if (!is_string($role) || $role === '') {
			throw new NetteX\InvalidArgumentException("Role must be a non-empty string.");

		} elseif ($need && !isset($this->roles[$role])) {
			throw new NetteX\InvalidStateException("Role '$role' does not exist.");
		}
	}
	...
	function addResource($resource, $parent = NULL)
	{
		...
		if (isset($this->resources[$resource])) {
			throw new NetteX\InvalidStateException("Resource '$resource' already exists in the list.");
		}
	...
	private function checkResource($resource, $need = TRUE)
	{
		if (!is_string($resource) || $resource === '') {
			throw new NetteX\InvalidArgumentException("Resource must be a non-empty string.");

		} elseif ($need && !isset($this->resources[$resource])) {
			throw new NetteX\InvalidStateException("Resource '$resource' does not exist.");
		}
	}
	...
	protected function setRule($toAdd, $type, $roles, $resources, $privileges, $assertion = NULL)
	{
		...
		$assertion = $assertion ? callbackX($assertion) : NULL;
...
class SimpleAuthenticator extends NetteX\Object implements IAuthenticator
{
...
use NetteX;use NetteX\Utils\Strings;use NetteX\Forms\Form;use NetteX\Utils\Html;

final class DefaultHelpers
{
	private static $helpers = array(
		'normalize' => 'NetteX\Utils\Strings::normalize',
		'toascii' => 'NetteX\Utils\Strings::toAscii',
		'webalize' => 'NetteX\Utils\Strings::webalize',
		'truncate' => 'NetteX\Utils\Strings::truncate',
		'lower' => 'NetteX\Utils\Strings::lower',
		'upper' => 'NetteX\Utils\Strings::upper',
		'firstupper' => 'NetteX\Utils\Strings::firstUpper',
		'capitalize' => 'NetteX\Utils\Strings::capitalize',
		'trim' => 'NetteX\Utils\Strings::trim',
		'padleft' => 'NetteX\Utils\Strings::padLeft',
		'padright' => 'NetteX\Utils\Strings::padRight',
		'replacere' => 'NetteX\Utils\Strings::replace',
		'url' => 'rawurlencode',
	...
	static function loader($helper)
	{
		if (method_exists(__CLASS__, $helper)) {
			return callbackX(__CLASS__, $helper);
	...
	static function escapeJs($s)
	{
		...
		return str_replace(']]>', ']]\x3E', NetteX\Utils\Json::encode($s));
	}
	...
	{
		if ($type === NULL) {
			$type = NetteX\Utils\MimeTypeDetector::fromString($data, NULL);
		}
...
class Template extends NetteX\Object implements ITemplate
{
	...
	function render()
	{
		$cache = new Caching\Cache($storage = $this->getCacheStorage(), 'NetteX.Template');
		$cached = $compiled = $cache->load($this->source);

		if ($compiled === NULL) {
			$compiled = $this->compile();
			$cache->save($this->source, $compiled, array(Caching\Cache::CONSTS => 'NetteX\Framework::REVISION'));
			$cache->release();
			$cached = $cache->load($this->source);
		}

		if ($cached !== NULL && $storage instanceof Caching\Storages\PhpFileStorage) {
			NetteX\Utils\LimitedScope::load($cached['file'], $this->getParams());
		} else {
			NetteX\Utils\LimitedScope::evaluate($compiled, $this->getParams());
		}
	}
	...
	function save($file)
	{
		if (file_put_contents($file, $this->__toString(TRUE)) === FALSE) {
			throw new NetteX\IOException("Unable to save file '$file'.");
		}
	}
	...
	{
		$callback = callbackX($callback);
		if (in_array($callback, $this->filters)) {
			throw new NetteX\InvalidStateException("Filter '$callback' was registered twice.");
		}
	...
	{
		$this->helpers[strtolower($name)] = callbackX($callback);
	}

	function registerHelperLoader($callback)
	{
		$this->helperLoaders[] = callbackX($callback);
	}
	...
	function add($name, $value)
	{
		if (array_key_exists($name, $this->params)) {
			throw new NetteX\InvalidStateException("The variable '$name' already exists.");
		}
...
class FileTemplate extends Template implements IFileTemplate
{
	...
	function setFile($file)
	{
		...
		if (!$this->file) {
			throw new NetteX\FileNotFoundException("Missing template file '$file'.");
		}
		return $this;
	}
	...
	{
		if ($this->file == NULL) {
			throw new NetteX\InvalidStateException("Template file name was not specified.");
		}

		$cache = new Caching\Cache($storage = $this->getCacheStorage(), 'NetteX.FileTemplate');
		...
		if ($compiled === NULL) {
			...
			$cache->save($this->file, $compiled, array(
				...
				Caching\Cache::CONSTS => 'NetteX\Framework::REVISION',
			));
			$cache->release();
	...
	static function get(array $arr, $key, $default = NULL)
	{
		foreach (is_array($key) ? $key : array($key) as $k) {
			if (is_array($arr) && array_key_exists($k, $arr)) {
				...
				if (func_num_args() < 3) {
					throw new NetteX\InvalidArgumentException("Missing item '$k'.");
				}
				return $default;
			}
		}
	...
	static function & getRef(& $arr, $key)
	{
		foreach (is_array($key) ? $key : array($key) as $k) {
			if (is_array($arr) || $arr === NULL) {
				...
				throw new NetteX\InvalidArgumentException('Traversed item is not an array.');
			}
		}
		return $arr;
	}
...
final class CriticalSection
{
	...
	static function enter()
	{
		if (self::$criticalSections) {
			throw new NetteX\InvalidStateException('Critical section has already been entered.');
		}

		$handle = substr(PHP_OS, 0, 3) === 'WIN'
			? @fopen(NETTEX_DIR . '/lockfile', 'w')
			: @fopen(__FILE__, 'r');

		if (!$handle) {
			throw new NetteX\InvalidStateException("Unable initialize critical section.");
		}
	...
	{
		if (!self::$criticalSections) {
			throw new NetteX\InvalidStateException('Critical section has not been initialized.');
		}
...
class Finder extends NetteX\Object implements \IteratorAggregate
{
	...
	{
		if ($this->paths) {
			throw new NetteX\InvalidStateException('Directory to search has already been specified.');
		}
		if (!is_array($path)) {
			$path = func_get_args();
		}
	...
	{
		if (!$this->paths) {
			throw new NetteX\InvalidStateException('Call in() or from() to specify directory to search.');
	...
	private function buildIterator($path)
	{
		if (PHP_VERSION_ID < 50301) {
			$iterator = new NetteX\Utils\RecursiveDirectoryIteratorFixed($path);
		} else {
		...
		if ($this->exclude) {
			...
			$iterator = new NetteX\Iterators\RecursiveFilter($iterator, function($file) use($filters) {
				if (!$file->isFile()) {
		...
		if ($this->groups) {
			$groups = $this->groups;
			$iterator = new NetteX\Iterators\Filter($iterator, function($file) use($groups) {
	...
	function size($operator, $size = NULL)
	{
		if (func_num_args() === 1) {
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?((?:\d*\.)?\d+)\s*(K|M|G|)B?$#i', $operator, $matches)) {
				throw new NetteX\InvalidArgumentException('Invalid size predicate format.');
			}
	...
	function date($operator, $date = NULL)
	{
		if (func_num_args() === 1) {
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?(.+)$#i', $operator, $matches)) {
				throw new NetteX\InvalidArgumentException('Invalid date predicate format.');
			}
			list(, $operator, $date) = $matches;
			$operator = $operator ? $operator : '=';
		}
		$date = NetteX\DateTime::from($date)->format('U');
	...
	static function compare($l, $operator, $r)
	{
		...
		throw new NetteX\InvalidArgumentException("Unknown operator $operator.");
	}
...
class Html extends NetteX\Object implements \ArrayAccess, \Countable, \IteratorAggregate
{
	...
	final function setName($name, $isEmpty = NULL)
	{
		if ($name !== NULL && !is_string($name)) {
			throw new NetteX\InvalidArgumentException("Name must be string or NULL, " . gettype($name) ." given.");
		}
	...
	final function setHtml($html)
	{
		if ($html === NULL) {
			...
			throw new NetteX\InvalidArgumentException("Textual content must be a scalar, " . gettype($html) ." given.");
	...
	function insert($index, $child, $replace = FALSE)
	{
		if ($child instanceof Html || is_scalar($child)) {
			...
			throw new NetteX\InvalidArgumentException("Child node must be scalar or Html object, " . (is_object($child) ? get_class($child) : gettype($child)) ." given.");
		}
	...
	final function getIterator($deep = FALSE)
	{
		if ($deep) {
			$deep = $deep > 0 ? \RecursiveIteratorIterator::SELF_FIRST : \RecursiveIteratorIterator::CHILD_FIRST;
			return new \RecursiveIteratorIterator(new NetteX\Iterators\Recursor(new \ArrayIterator($this->children)), $deep);

		} else {
			return new NetteX\Iterators\Recursor(new \ArrayIterator($this->children));
		}
	}
...
final class MimeTypeDetector
{
	...
	{
		if (!is_file($file)) {
			throw new NetteX\FileNotFoundException("File '$file' not found.");
		}
...
class Neon extends NetteX\Object
{
	const BLOCK = 1;
	...
	{
		if (!is_string($input)) {
			throw new NetteX\InvalidArgumentException("Argument must be a string, " . gettype($input) . " given.");
		}
		if (!self::$tokenizer) {
	...
	private function parse($indent = NULL, $result = NULL)
	{
		...
		for (; $n < $count; $n++) {
			...
			if ($t === ',') {
				...
				if (!isset($tokens[$n]) || $tokens[$n] !== $endBracket) {
					$this->error();
				}
...
class Paginator extends NetteX\Object
{
...
class Strings
{
	...
	static function replace($subject, $pattern, $replacement = NULL, $limit = -1)
	{
		...
		if (is_object($replacement) || is_array($replacement)) {
			if ($replacement instanceof NetteX\Callback) {
				$replacement = $replacement->getNative();
			}
			if (!is_callable($replacement, FALSE, $textual)) {
				NetteX\Diagnostics\Debugger::catchError($foo);
				throw new NetteX\InvalidStateException("Callback '$textual' is not callable.");
			}
...
namespace  {
 ...
 NetteX\Diagnostics\Debugger::_init();

NetteX\Utils\SafeStream::register();

function callbackX($callback, $m = NULL)
{
	return ($m === NULL && $callback instanceof NetteX\Callback) ? $callback : new NetteX\Callback($callback, $m);
}

function dumpX($var)
{
	foreach (func_get_args() as $arg) NetteX\Diagnostics\Debugger::dump($arg);
	return $var;
}
}