...
namespace {/**
 * NetteX Framework (version 2.0-dev released on 2011-05-28, http://nette.org)
 *
...
callbackX($callback,$m=NULL){return($m===NULL&&$callback
instanceof
NetteX\Callback)?$callback:new
NetteX\Callback($callback,$m);}function
dumpX($var){foreach(func_get_args()as$arg)NetteX\Diagnostics\Debugger::dump($arg);return$var;}}namespace NetteX\Diagnostics{use
NetteX;interface
IBarPanel{function
getTab();function
getPanel();}}namespace NetteX\Application{use
NetteX;interface
IPresenter{function
...
constructUrl(Request$appRequest,NetteX\Http\Url$refUrl);}}namespace NetteX{use
NetteX;interface
IFreezable{function
freeze();function
...
IServiceBuilder{function
createService(IContainer$container);}}namespace NetteX\Forms{use
NetteX;interface
IControl{function
...
getAuthorizationHandler();}}namespace NetteX\Latte{use
NetteX;interface
IMacro{function
initialize();function
finalize();function
nodeOpened(MacroNode$node);function
nodeClosed(MacroNode$node);}}namespace NetteX\Localization{use
NetteX;interface
...
new
NetteX\FatalErrorException($error['message'],0,$error['type'],$error['file'],$error['line'],NULL);}return$res;}static
function
...
function
tryLoad($type);}}namespace NetteX\Utils{use
NetteX;final
class
SafeStream{const
...
__construct(NetteX\DI\IContainer$context){$this->context=$context;}function
run(){$httpRequest=$this->context->httpRequest;$httpResponse=$this->context->httpResponse;if($this->allowedMethods){$method=$httpRequest->getMethod();if(!in_array($method,$this->allowedMethods,TRUE)){$httpResponse->setCode(NetteX\Http\IResponse::S501_NOT_IMPLEMENTED);$httpResponse->setHeader('Allow',implode(',',$this->allowedMethods));echo'<h1>Method '.htmlSpecialChars($method).' is not implemented</h1>';return;}}$request=NULL;$repeatedError=FALSE;do{try{if(count($this->requests)>self::$maxLoop){throw
new
ApplicationException('Too many loops detected in application life cycle.');}if(!$request){$this->onStartup($this);$router=$this->getRouter();Diagnostics\RoutingPanel::initialize($this,$httpRequest);$request=$router->match($httpRequest);if(!$request
instanceof
...
ApplicationException('An error occurred while executing error-presenter',0,$e);}if(!$httpResponse->isSent()){$httpResponse->setCode($e
instanceof
...
getRouter(){return$this->context->router;}function
getPresenterFactory(){return$this->context->presenterFactory;}function
storeRequest($expiration='+ 10 minutes'){$session=$this->context->session->getNamespace('NetteX.Application/requests');do{$key=NetteX\Utils\Strings::random(5);}while(isset($session[$key]));$session[$key]=end($this->requests);$session->setExpiration($expiration,$key);return$key;}function
restoreRequest($key){$session=$this->context->session->getNamespace('NetteX.Application/requests');if(isset($session[$key])){$request=clone$session[$key];unset($session[$key]);$request->setFlag(Request::RESTORED,TRUE);$this->presenter->sendResponse(new
Responses\ForwardResponse($request));}}}}namespace NetteX\Application\Diagnostics{use
NetteX;use
...
NetteX\Object
implements
NetteX\Diagnostics\IBarPanel{private$router;private$httpRequest;private$routers=array();private$request;static
function
initialize(NetteX\Application\Application$application,NetteX\Http\IRequest$httpRequest){Debugger::$bar->addPanel(new
self($application->getRouter(),$httpRequest));Debugger::$blueScreen->addPanel(function($e)use($application){if($e===NULL){return
array('tab'=>'NetteX Application','panel'=>'<h3>Requests</h3>'.NetteX\Diagnostics\Helpers::clickableDump($application->getRequests()).'<h3>Presenter</h3>'.NetteX\Diagnostics\Helpers::clickableDump($application->getPresenter()));}});}function
__construct(NetteX\Application\IRouter$router,NetteX\Http\IRequest$httpRequest){$this->router=$router;$this->httpRequest=$httpRequest;}function
...
__construct($baseDir,NetteX\DI\IContainer$context){$this->baseDir=$baseDir;$this->context=$context;}function
...
getPresenterClass(&$name){if(isset($this->cache[$name])){list($class,$name)=$this->cache[$name];return$class;}if(!is_string($name)||!NetteX\Utils\Strings::match($name,"#^[a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff:]*$#")){throw
new
...
new
InvalidStateException("Cannot modify a frozen object $class.");}}}}namespace NetteX\Application{use
NetteX;final
class
Request
extends
...
NetteX\Object
implements
NetteX\Application\IResponse{private$url;private$code;function
...
constructUrl(Application\Request$appRequest,NetteX\Http\Url$refUrl){return
NULL;}function
getDefaults(){return$this->defaults;}}use
NetteX\Utils\Strings;class
Route
extends
NetteX\Object
implements
...
new
NetteX\InvalidArgumentException("Second argument must be array or string in format Presenter:action, '$metadata' given.");}$metadata=array(self::PRESENTER_KEY=>substr($metadata,0,$a),'action'=>$a===strlen($metadata)-1?Application\UI\Presenter::DEFAULT_ACTION:substr($metadata,$a+1));}elseif($metadata
instanceof\Closure||$metadata
instanceof
NetteX\Callback){$metadata=array(self::PRESENTER_KEY=>'NetteX:Simple','callback'=>$metadata);}$this->flags=$flags|self::$defaultFlags;$this->setMask($mask,$metadata);}function
match(NetteX\Http\IRequest$httpRequest){$url=$httpRequest->getUrl();if($this->type===self::HOST){$path='//'.$url->getHost().$url->getPath();}elseif($this->type===self::RELATIVE){$basePath=$url->getBasePath();if(strncmp($url->getPath(),$basePath,strlen($basePath))!==0){return
...
constructUrl(Application\Request$appRequest,NetteX\Http\Url$refUrl){if($this->flags&self::ONE_WAY){return
...
new
OutOfRangeException("Offset invalid or out of range");}array_splice($this->list,(int)$index,1);}}}namespace NetteX\Application\Routers{use
NetteX;class
RouteList
extends
NetteX\ArrayList
...
NULL;}function
constructUrl(NetteX\Application\Request$appRequest,NetteX\Http\Url$refUrl){if($this->cachedRoutes===NULL){$routes=array();$routes['*']=array();foreach($this
...
match(NetteX\Http\IRequest$httpRequest){if($httpRequest->getUrl()->getPathInfo()!==''){return
...
new
Application\Request($presenter,$httpRequest->getMethod(),$params,$httpRequest->getPost(),$httpRequest->getFiles(),array(Application\Request::SECURED=>$httpRequest->isSecured()));}function
constructUrl(Application\Request$appRequest,NetteX\Http\Url$refUrl){$params=$appRequest->getParams();$presenter=$appRequest->getPresenterName();if(strncasecmp($presenter,$this->module,strlen($this->module))===0){$params[self::PRESENTER_KEY]=substr($presenter,strlen($this->module));}else{return
...
getDefaults(){return$this->defaults;}}}namespace NetteModule{use
NetteX;use
...
run(Application\Request$request){$this->request=$request;$params=$request->getParams();if(isset($params['callback'])){$params['presenter']=$this;return
callbackX($params['callback'])->invokeNamedArgs($params);}}function
redirectUrl($url,$code=NULL){if(!$code){$code=$this->context->httpRequest->isMethod('post')?Http\IResponse::S303_POST_GET:Http\IResponse::S302_FOUND;}return
new
Responses\RedirectResponse($url,$code);}function
setContext(NetteX\DI\IContainer$context){$this->context=$context;return$this;}final
function
getContext(){return$this->context;}}}namespace NetteX\Application\UI{use
NetteX;class
BadSignalException
extends
...
new
NetteX\UnexpectedValueException("Method $class::$method() did not return or create the desired component.");}return$component;}}final
function
...
new
NetteX\InvalidStateException("Persistent parameter must be scalar or array, $class::\$$nm is ".gettype($val));}else{if(isset($meta['def'])){settype($val,gettype($meta['def']));if($val===$meta['def']){$val=NULL;}}else{if((string)$val===''){$val=NULL;}}$params[$nm]=$val;}}}final
function
...
instanceof
NetteX\Templating\ITemplate&&$value!==NULL){$class2=get_class($value);$class=get_class($this);throw
new
NetteX\UnexpectedValueException("Object returned by $class::createTemplate() must be instance of NetteX\\Templating\\ITemplate, '$class2' given.");}$this->template=$value;}return$this->template;}protected
function
createTemplate(){$template=new
NetteX\Templating\FileTemplate;$presenter=$this->getPresenter(FALSE);$template->onPrepareFilters[]=callbackX($this,'templatePrepareFilters');$template->control=$this;$template->presenter=$presenter;if($presenter
instanceof
Presenter){$template->setCacheStorage($presenter->getContext()->templateCacheStorage);$template->user=$presenter->getUser();$template->netteHttpResponse=$presenter->getHttpResponse();$template->netteCacheStorage=$presenter->getContext()->cacheStorage;$template->baseUri=$template->baseUrl=rtrim($presenter->getHttpRequest()->getUrl()->getBaseUrl(),'/');$template->basePath=preg_replace('#https?://[^/]+#A','',$template->baseUrl);if($presenter->hasFlashSession()){$id=$this->getParamId('flash');$template->flashes=$presenter->getFlashSession()->$id;}}if(!isset($template->flashes)||!is_array($template->flashes)){$template->flashes=array();}$template->registerHelper('escape','NetteX\Templating\DefaultHelpers::escapeHtml');$template->registerHelper('escapeUrl','rawurlencode');$template->registerHelper('stripTags','strip_tags');$template->registerHelper('nl2br','nl2br');$template->registerHelper('substr','iconv_substr');$template->registerHelper('repeat','str_repeat');$template->registerHelper('replaceRE','NetteX\Utils\Strings::replace');$template->registerHelper('implode','implode');$template->registerHelper('number','number_format');$template->registerHelperLoader('NetteX\Templating\DefaultHelpers::loader');return$template;}function
...
addProtection($message=NULL,$timeout=NULL){$session=$this->getSession()->getNamespace('NetteX.Forms.Form/CSRF');$key="key$timeout";if(isset($session->$key)){$token=$session->$key;}else{$session->$key=$token=NetteX\Utils\Strings::random();}$session->setExpiration($timeout,$key);$this[self::PROTECTOR_ID]=new
...
function
receiveHttpData(){$httpRequest=$this->getHttpRequest();if(strcasecmp($this->getMethod(),$httpRequest->getMethod())){return;}if($httpRequest->isMethod('post')){$data=NetteX\Utils\Arrays::mergeTree($httpRequest->getPost(),$httpRequest->getFiles());}else{$data=$httpRequest->getQuery();}if($tracker=$this->getComponent(self::TRACKER_ID,FALSE)){if(!isset($data[self::TRACKER_ID])||$data[self::TRACKER_ID]!==$tracker->getValue()){return;}}return$data;}function
...
NetteX\Utils\Arrays::mergeTree($request->getPost(),$request->getFiles());}else{return$request->getParams();}}function
...
run(Application\Request$request){try{$this->request=$request;$this->payload=(object)NULL;$this->setParent($this->getParent(),$request->getPresenterName());$this->initGlobalParams();$this->checkRequirements($this->getReflection());$this->startup();if(!$this->startupCheck){$class=$this->getReflection()->getMethod('startup')->getDeclaringClass()->getName();throw
new
...
changeAction($action){if(NetteX\Utils\Strings::match($action,"#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*$#")){$this->action=$action;$this->view=$action;}else{throw
new
...
instanceof
NetteX\Templating\IFileTemplate&&!$template->getFile()){$files=$this->formatTemplateFiles();foreach($files
...
canonicalize(){if(!$this->isAjax()&&($this->request->isMethod('get')||$this->request->isMethod('head'))){$url=$this->createRequest($this,$this->action,$this->getGlobalState()+$this->request->params,'redirectX');if($url!==NULL&&!$this->getHttpRequest()->getUrl()->isEqual($url)){$this->sendResponse(new
...
function
handleInvalidLink($e){if(self::$invalidLinkMode===self::INVALID_LINK_SILENT){return'#';}elseif(self::$invalidLinkMode===self::INVALID_LINK_WARNING){return'error: '.$e->getMessage();}else{throw$e;}}static
function
...
getFlashSession(){if(empty($this->params[self::FLASH_KEY])){$this->params[self::FLASH_KEY]=NetteX\Utils\Strings::random(4);}return$this->getSession('NetteX.Application.Flash/'.$this->params[self::FLASH_KEY]);}function
setContext(NetteX\DI\IContainer$context){$this->context=$context;return$this;}final
function
...
function
getHttpRequest(){return$this->context->httpRequest;}protected
function
getHttpResponse(){return$this->context->httpResponse;}protected
function
getHttpContext(){return$this->context->httpContext;}function
getApplication(){return$this->context->application;}function
getSession($namespace=NULL){$handler=$this->context->session;return$namespace===NULL?$handler:$handler->getNamespace($namespace);}function
getUser(){return$this->context->user;}}}namespace NetteX\Reflection{use
NetteX;use
...
FALSE;}function
setExtensionMethod($name,$callback){$l=&self::$extMethods[strtolower($name)];$l[strtolower($this->getName())]=callbackX($callback);$l['']=NULL;return$this;}function
...
load($key){$key=is_scalar($key)?(string)$key:serialize($key);if($this->key===$key){return$this->data;}$this->key=$key;$this->data=$this->storage->read($this->namespace.md5($key));return$this->data;}function
...
call($function){$key=func_get_args();if($this->load($key)===NULL){array_shift($key);return$this->save($this->key,call_user_func_array($function,$key));}else{return$this->data;}}function
start($key){if($this->offsetGet($key)===NULL){return
new
OutputHelper($this,$key);}else{echo$this->data;}}function
offsetSet($key,$data){$this->save($key,$data);}function
offsetGet($key){return$this->load($key);}function
offsetExists($key){return$this->load($key)!==NULL;}function
...
OutputHelper
extends
NetteX\Object{public$dependencies;private$cache;private$key;function
__construct(Cache$cache,$key){$this->cache=$cache;$this->key=$key;ob_start();}function
end(){if($this->cache===NULL){throw
new
NetteX\InvalidStateException('Output cache has already been saved.');}$this->cache->save($this->key,ob_get_flush(),$this->dependencies);$this->cache=NULL;}}}namespace NetteX\Caching\Storages{use
NetteX;class
DevNullStorage
extends
NetteX\Object
...
static$useDirectories=TRUE;private$dir;private$useDirs;private$journal;function
__construct($dir,IJournal$journal=NULL){$this->dir=realpath($dir);if($this->dir===FALSE){throw
new
NetteX\DirectoryNotFoundException("Directory '$dir' not found.");}$this->useDirs=(bool)self::$useDirectories;$this->journal=$journal;if(mt_rand()/mt_getrandmax()<self::$gcProbability){$this->clean(array());}}function
...
clean(array$conds){if(!empty($conds[NetteX\Caching\Cache::ALL])){$this->data=array();}}}class
PhpFileStorage
extends
...
parent::getCacheFile(substr_replace($key,trim(strtr($this->hint,'\\/@','.._'),'.').'-',strpos($key,NetteX\Caching\Cache::NAMESPACE_SEPARATOR)+1,0)).'.php';}}}namespace NetteX{use
NetteX;class
ArrayHash
...
new
NetteX\Reflection\Method($this->cb[0],$this->cb[1]);}else{return
new
NetteX\Reflection\GlobalFunction($this->cb);}}function
isStatic(){return
is_array($this->cb)?is_string($this->cb[0]):is_string($this->cb);}function
__toString(){if($this->cb
instanceof\Closure){return'{closure}';}elseif(is_string($this->cb)&&$this->cb[0]==="\0"){return'{lambda}';}else{is_callable($this->cb,TRUE,$textual);return$textual;}}}use
NetteX\Caching\Cache;use
NetteX\DI;class
Configurator
extends
Object{public
static$instance;public$defaultConfigFile='%appDir%/config.neon';private$container;function
__construct($containerClass='NetteX\DI\Container'){self::$instance=$this;$this->container=new$containerClass;foreach(get_class_methods($this)as$name){if(substr($name,0,13)==='createService'){$this->container->addService(strtolower($name[13]).substr($name,14),array(get_called_class(),$name));}}$this->container->params=new
ArrayHash;defined('APP_DIR')&&$this->container->params['appDir']=realpath(APP_DIR);defined('LIBS_DIR')&&$this->container->params['libsDir']=realpath(LIBS_DIR);defined('TEMP_DIR')&&$this->container->params['tempDir']=realpath(TEMP_DIR);$this->container->params['productionMode']=self::detectProductionMode();$this->container->params['consoleMode']=PHP_SAPI==='cli';}function
getContainer(){return$this->container;}function
loadConfig($file,$section=NULL){if($file===NULL){$file=$this->defaultConfigFile;}$container=$this->container;$file=$container->expand($file);if(!is_file($file)){$file=preg_replace('#\.neon$#','.ini',$file);}if($section===NULL){if(PHP_SAPI==='cli'){$section=Environment::CONSOLE;}else{$section=$container->params['productionMode']?Environment::PRODUCTION:Environment::DEVELOPMENT;}}$cache=new
Cache($container->templateCacheStorage,__CLASS__);$cacheKey=array((array)$container->params,$file,$section);$cached=$cache->load($cacheKey);if($cached){ require $cached['file'];fclose($cached['handle']);return;}$config=NetteX\Config\Config::fromFile($file,$section);$code="<?php\n// source file $file\n\n";foreach(array('service','variable')as$item){if(isset($config[$item])){trigger_error(basename($file).": Section '$item' is deprecated; use plural form '{$item}s' instead.",E_USER_WARNING);$config[$item.'s']=$config[$item];unset($config[$item]);}}while(!empty($config['variables'])){$old=$config['variables'];foreach($config['variables']as$key=>$value){try{$code.=$this->generateCode('$container->params[?] = ?',$key,$container->params[$key]=$container->expand($value));unset($config['variables'][$key]);}catch(NetteX\InvalidArgumentException$e){}}if($old===$config['variables']){throw
new
InvalidStateException("Circular reference detected for variables: ".implode(', ',array_keys($old)).".");}}unset($config['variables']);if(isset($config['services'])){foreach($config['services']as$key=>&$def){if(preg_match('#^NetteX\\\\.*\\\\I?([a-zA-Z]+)$#',strtr($key,'-','\\'),$m)){$m[1][0]=strtolower($m[1][0]);trigger_error(basename($file).": service name '$key' has been renamed to '$m[1]'",E_USER_WARNING);$key=$m[1];}if(is_scalar($def)){$def=array('class'=>$def);}if(method_exists(get_called_class(),"createService$key")){$container->removeService($key);if(!isset($def['factory'])&&!isset($def['class'])){$def['factory']=array(get_called_class(),"createService$key");}}if(isset($def['option'])){$def['arguments'][]=$def['option'];}if(!empty($def['run'])){$def['tags']=array('run');}}$builder=new
...
as$key=>$value){$code.=$this->generateCode('$container->params[?] = '.(is_array($value)?'NetteX\ArrayHash::from(?)':'?'),$key,$value);}$code.=self::preloadEnvironment($container);$code.='foreach ($container->getServiceNamesByTag("run") as $name => $foo) { $container->getService($name); }'."\n";$cache->save($cacheKey,$code,array(Cache::FILES=>$file));NetteX\Utils\LimitedScope::evaluate($code,array('container'=>$container));}static
function
detectProductionMode(){if(!isset($_SERVER['SERVER_ADDR'])&&!isset($_SERVER['LOCAL_ADDR'])){return
...
TRUE;}}return
FALSE;}function
configurePhp($name,$value){if(!is_scalar($value)){throw
new
NetteX\InvalidStateException("Configuration value for directive '$name' is not scalar.");}switch($name){case'include_path':return$this->generateCode('set_include_path',str_replace(';',PATH_SEPARATOR,$value));case'ignore_user_abort':return$this->generateCode('ignore_user_abort',$value);case'max_execution_time':return$this->generateCode('set_time_limit',$value);case'date.timezone':return$this->generateCode('date_default_timezone_set',$value);}if(function_exists('ini_set')){return$this->generateCode('ini_set',$name,$value);}elseif(ini_get($name)!=$value){throw
new
NetteX\NotSupportedException('Required function ini_set() is disabled.');}}private
static
function
generateCode($statement){$args=func_get_args();unset($args[0]);foreach($args
...
function
createServiceApplication(DI\Container$container,array$options=NULL){$context=new
DI\Container;$context->addService('httpRequest',$container->httpRequest);$context->addService('httpResponse',$container->httpResponse);$context->addService('session',$container->session);$context->addService('presenterFactory',$container->presenterFactory);$context->addService('router',$container->router);NetteX\Application\UI\Presenter::$invalidLinkMode=$container->params['productionMode']?NetteX\Application\UI\Presenter::INVALID_LINK_SILENT:NetteX\Application\UI\Presenter::INVALID_LINK_WARNING;$class=isset($options['class'])?$options['class']:'NetteX\Application\Application';$application=new$class($context);$application->catchExceptions=$container->params['productionMode'];if($container->session->exists()){$application->onStartup[]=function()use($container){$container->session->start();};}return$application;}static
function
createServicePresenterFactory(DI\Container$container){return
new
NetteX\Application\PresenterFactory($container->params['appDir'],$container);}static
function
createServiceRouter(DI\Container$container){return
new
NetteX\Application\Routers\RouteList;}static
function
createServiceHttpRequest(){$factory=new
NetteX\Http\RequestFactory;$factory->setEncoding('UTF-8');return$factory->createHttpRequest();}static
function
createServiceHttpResponse(){return
new
NetteX\Http\Response;}static
function
createServiceHttpContext(DI\Container$container){return
new
NetteX\Http\Context($container->httpRequest,$container->httpResponse);}static
function
createServiceSession(DI\Container$container){return
new
NetteX\Http\Session($container->httpRequest,$container->httpResponse);}static
function
createServiceUser(DI\Container$container){$context=new
DI\Container;$context->addService('authenticator',function()use($container){return$container->authenticator;});$context->addService('authorizator',function()use($container){return$container->authorizator;});$context->addService('session',$container->session);return
new
NetteX\Http\User($context);}static
function
createServiceCacheStorage(DI\Container$container){$dir=$container->expand('%tempDir%/cache');umask(0000);@mkdir($dir,0777);return
new
NetteX\Caching\Storages\FileStorage($dir,$container->cacheJournal);}static
function
createServiceTemplateCacheStorage(DI\Container$container){$dir=$container->expand('%tempDir%/cache');umask(0000);@mkdir($dir,0777);return
new
NetteX\Caching\Storages\PhpFileStorage($dir);}static
function
createServiceCacheJournal(DI\Container$container){return
new
NetteX\Caching\Storages\FileJournal($container->params['tempDir']);}static
function
createServiceMailer(DI\Container$container,array$options=NULL){if(empty($options['smtp'])){return
new
NetteX\Mail\SendmailMailer;}else{return
new
NetteX\Mail\SmtpMailer($options);}}static
function
createServiceRobotLoader(DI\Container$container,array$options=NULL){$loader=new
NetteX\Loaders\RobotLoader;$loader->autoRebuild=isset($options['autoRebuild'])?$options['autoRebuild']:!$container->params['productionMode'];$loader->setCacheStorage($container->cacheStorage);if(isset($options['directory'])){$loader->addDirectory($options['directory']);}else{foreach(array('appDir','libsDir')as$var){if(isset($container->params[$var])){$loader->addDirectory($container->params[$var]);}}}$loader->register();return$loader;}static
function
preloadEnvironment(DI\Container$container){$code='';$dir=$container->expand('%tempDir%/cache');umask(0000);@mkdir($dir,0777);$uniq=uniqid('_',TRUE);umask(0000);if(!@mkdir("$dir/$uniq",0777)){throw
new
NetteX\InvalidStateException("Unable to write to directory '$dir'. Make this directory writable.");}$useDirs=@file_put_contents("$dir/$uniq/_",'')!==FALSE;@unlink("$dir/$uniq/_");@rmdir("$dir/$uniq");$code.=self::generateCode('NetteX\Caching\Storages\FileStorage::$useDirectories = ?',$useDirs);return$code;}}class
DateTime
...
function
getConfigurator(){if(self::$configurator===NULL){self::$configurator=Configurator::$instance?:new
Configurator;}return
self::$configurator;}static
function
isConsole(){return
self::getContext()->params['consoleMode'];}static
function
isProduction(){return
self::getContext()->params['productionMode'];}static
function
setProductionMode($value=TRUE){self::getContext()->params['productionMode']=(bool)$value;}static
function
setVariable($name,$value,$expand=TRUE){if($expand&&is_string($value)){$value=self::getContext()->expand($value);}self::getContext()->params[$name]=$value;}static
function
getVariable($name,$default=NULL){if(isset(self::getContext()->params[$name])){return
...
function
getVariables(){return
self::getContext()->params;}static
function
expand($s){return
self::getContext()->expand($s);}static
function
setContext(DI\IContainer$context){self::$context=$context;}static
function
getContext(){if(self::$context===NULL){self::$context=self::getConfigurator()->getContainer();}return
self::$context;}static
function
...
function
getHttpRequest(){return
self::getContext()->httpRequest;}static
function
getHttpContext(){return
self::getContext()->httpContext;}static
function
getHttpResponse(){return
self::getContext()->httpResponse;}static
function
getApplication(){return
self::getContext()->application;}static
function
getUser(){return
self::getContext()->user;}static
function
getRobotLoader(){return
self::getContext()->robotLoader;}static
function
getCache($namespace=''){return
new
Caching\Cache(self::getContext()->cacheStorage,$namespace);}static
function
getSession($namespace=NULL){return$namespace===NULL?self::getContext()->session:self::getContext()->session->getNamespace($namespace);}static
function
loadConfig($file=NULL,$section=NULL){self::getConfigurator()->loadConfig($file,$section);return
self::getContext()->params;}static
function
getConfig($key=NULL,$default=NULL){$params=self::getContext()->params;if(func_num_args()){return
isset($params[$key])?$params[$key]:$default;}else{return$params;}}}final
class
Framework{const
NAME='NetteX Framework',VERSION='2.0-beta',REVISION='83ba329 released on 2011-05-28';public
...
as$handler){callbackX($handler)->invokeArgs($args);}}return
...
isset(self::$methods[$class]['get'.$name])||isset(self::$methods[$class]['is'.$name]);}}}namespace NetteX\ComponentModel{use
NetteX;class
...
NetteX;class
Config{private
static$extensions=array('ini'=>'NetteX\Config\IniAdapter','neon'=>'NetteX\Config\NeonAdapter');final
function
__construct(){throw
new
NetteX\StaticClassException;}static
...
new
NetteX\InvalidStateException("There is not section [$section] in file '$file'.");}$data=$data[$section];}return$data;}static
function
save($config,$file){$extension=strtolower(pathinfo($file,PATHINFO_EXTENSION));if(!isset(self::$extensions[$extension])){throw
new
NetteX\InvalidArgumentException("Unknown file extension '$file'.");}return
call_user_func(array(self::$extensions[$extension],'save'),$config,$file);}}final
class
IniAdapter
implements
IAdapter{public
...
new
NetteX\InvalidStateException("Invalid key '$key' in section [$secName] in file '$file'.");}}$cursor=$val;}$secData=$tmp;}$parts=$separator?explode($separator,strtr($secName,':',$separator)):array($secName);if(count($parts)>1){$parent=trim($parts[1]);if(!isset($data[$parent])||!is_array($data[$parent])){throw
new
NetteX\InvalidStateException("Missing parent section [$parent] in file '$file'.");}$secData=NetteX\Utils\Arrays::mergeTree($secData,$data[$parent]);$secName=trim($parts[0]);if($secName===''){throw
new
NetteX\InvalidStateException("Invalid empty section name in file '$file'.");}}}if(self::$keySeparator){$cursor=&$data;foreach(explode(self::$keySeparator,$secName)as$part){if(!isset($cursor[$part])||is_array($cursor[$part])){$cursor=&$cursor[$part];}else{throw
new
NetteX\InvalidStateException("Invalid section [$secName] in file '$file'.");}}}else{$cursor=&$data[$secName];}if(is_array($secData)&&is_array($cursor)){$secData=NetteX\Utils\Arrays::mergeTree($secData,$cursor);}$cursor=$secData;}return$data;}static
function
...
new
NetteX\InvalidStateException("Invalid section '$secName'.");}$output[]="[$secName]";self::build($secData,$output,'');$output[]='';}if(!file_put_contents($file,implode(PHP_EOL,$output))){throw
new
...
as$secName=>$secData){if($secData===NULL){$secData=array();}if(is_array($secData)){$parts=$separator?explode($separator,$secName):array($secName);if(count($parts)>1){$parent=trim($parts[1]);if(!isset($data[$parent])||!is_array($data[$parent])){throw
new
NetteX\InvalidStateException("Missing parent section '$parent' in file '$file'.");}$secData=NetteX\Utils\Arrays::mergeTree($secData,$data[$parent]);$secName=trim($parts[0]);if($secName===''){throw
new
NetteX\InvalidStateException("Invalid empty section name in file '$file'.");}}}$data[$secName]=$secData;}return$data;}static
function
...
Reflection\DatabaseReflection;Diagnostics\ConnectionPanel::initialize($this);}function
...
NetteX;use
NetteX\Database\Connection;use
NetteX\Diagnostics\Debugger;class
ConnectionPanel
extends
NetteX\Object
implements
NetteX\Diagnostics\IBarPanel{static
public$maxLength=1000;public$totalTime=0;public$queries=array();public$name;public$explain=TRUE;public$disabled=FALSE;static
function
initialize(Connection$connection){if(!Debugger::$productionMode){$panel=new
static;$connection->onQuery[]=callbackX($panel,'logQuery');Debugger::$bar->addPanel($panel);Debugger::$blueScreen->addPanel(callbackX($panel,'renderException'),__CLASS__);}}function
...
array('tab'=>'SQL','panel'=>Connection::highlightSql($e->queryString));}}function
...
as$col){$s.="<td>{$h($col)}</td>";}$s.="</tr>";}$s.="</table>";}if($source){$s.=NetteX\Diagnostics\Helpers::editorLink($source[0],$source[1])->class('nette-DbConnectionPanel-source');}$s.='</td><td>';foreach($params
...
NetteX\Object
implements
NetteX\Database\ISupplementalDriver{public$supports=array('meta'=>TRUE);private$connection;function
...
NetteX\Object
implements
NetteX\Database\ISupplementalDriver{public$supports=array('meta'=>TRUE);private$connection;private$fmtDateTime;function
...
NetteX\Object
implements
NetteX\Database\ISupplementalDriver{public$supports=array('meta'=>FALSE);private$connection;private$fmtDateTime;function
__construct(NetteX\Database\Connection$connection,array$options){$this->connection=$connection;$this->fmtDateTime=isset($options['formatDateTime'])?$options['formatDateTime']:'U';}function
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$cmd=strtoupper(substr(ltrim($sql),0,6));$this->arrayMode=$cmd==='INSERT'||$cmd==='REPLAC'?'values':'assoc';$sql=NetteX\Utils\Strings::replace($sql,'~\'.*?\'|".*?"|:[a-zA-Z0-9_]+:|\?~s',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
array($sql,$this->remaining);}function
callbackX($m){$m=$m[0];if($m[0]==="'"||$m[0]==='"'){return$m;}elseif($m[0]==='?'){return$this->formatValue($this->params[$this->counter++]);}elseif($m[0]===':'){$s=substr($m,1,-1);return
...
normalizeRow($row){if($this->types===NULL){$this->types=array();if($this->connection->getSupplementalDriver()->supports['meta']){foreach($row
as$key=>$foo){$type=$this->getColumnMeta(count($this->types));if(isset($type['native_type'])){$this->types[$key]=Reflection\DatabaseReflection::detectType($type['native_type']);}}}}foreach($this->types
...
__destruct(){if($this->connection->cache&&!$this->select&&$this->rows!==NULL){$accessed=$this->accessed;if(is_array($accessed)){$accessed=array_filter($accessed);}$this->connection->cache->save(array(__CLASS__,$this->name,$this->conditions),$accessed);}$this->rows=NULL;}function
...
getSql(){$join=$this->createJoins(implode(',',$this->conditions),TRUE)+$this->createJoins(implode(',',$this->select).",$this->group,$this->having,".implode(',',$this->order));if($this->rows===NULL&&$this->connection->cache&&!is_string($this->prevAccessed)){$this->accessed=$this->prevAccessed=$this->connection->cache->load(array(__CLASS__,$this->name,$this->conditions));}$prefix=$join?"$this->delimitedName.":'';if($this->select){$cols=implode(', ',$this->select);}elseif($this->prevAccessed){$cols=$prefix.implode(', '.$prefix,array_map(array($this->connection->getSupplementalDriver(),'delimite'),array_keys($this->prevAccessed)));}else{$cols=$prefix.'*';}return"SELECT{$this->topString()} $cols FROM $this->delimitedName".implode($join).$this->whereString();}protected
function
...
implements
IContainer{const
TAG_TYPEHINT='typeHint';public$params=array();private$registry=array();private$factories=array();private$tags=array();private$creating;function
addService($name,$service,$tags=NULL){$this->updating();if(!is_string($name)||$name===''){throw
new
NetteX\InvalidArgumentException("Service name must be a non-empty string, ".gettype($name)." given.");}if(isset($this->registry[$name])||method_exists($this,"createService$name")){throw
new
NetteX\InvalidStateException("Service '$name' has already been registered.");}if(is_string($tags)){$tags=array(self::TAG_TYPEHINT=>array($tags));}elseif(is_array($tags)){foreach($tags
...
ServiceBuilder($service);}if($service
instanceof
IServiceBuilder){$factory=array($service,'createService');}elseif(is_object($service)&&!$service
instanceof\Closure&&!$service
instanceof
NetteX\Callback){$this->registry[$name]=$service;$this->tags[$name]=$tags;return$this;}else{$factory=$service;}$this->factories[$name]=array(callbackX($factory));$this->tags[$name]=$tags;$this->registry[$name]=&$this->factories[$name][1];return$service;}function
removeService($name){$this->updating();unset($this->registry[$name],$this->factories[$name]);}function
getService($name){if(isset($this->registry[$name])){return$this->registry[$name];}elseif(isset($this->creating[$name])){throw
new
NetteX\InvalidStateException("Circular reference detected for services: ".implode(', ',array_keys($this->creating)).".");}if(isset($this->factories[$name])){list($factory)=$this->factories[$name];if(!$factory->isCallable()){throw
new
NetteX\InvalidStateException("Unable to create service '$name', factory '$factory' is not callable.");}$this->creating[$name]=TRUE;try{$service=$factory($this);}catch(\Exception$e){}}elseif(method_exists($this,$factory='createService'.ucfirst($name))){$this->creating[$name]=TRUE;try{$service=$this->$factory();}catch(\Exception$e){}}else{throw
new
...
new
NetteX\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not object.");}elseif(isset($this->tags[$name][self::TAG_TYPEHINT][0])&&!$service
instanceof$this->tags[$name][self::TAG_TYPEHINT][0]){throw
new
NetteX\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not '{$this->tags[$name][self::TAG_TYPEHINT][0]}' type.");}unset($this->factories[$name]);return$this->registry[$name]=$service;}function
getServiceByType($type){foreach($this->registry
as$name=>$service){if(isset($this->tags[$name][self::TAG_TYPEHINT][0])?!strcasecmp($this->tags[$name][self::TAG_TYPEHINT][0],$type):$service
instanceof$type){$found[]=$name;}}if(!isset($found)){throw
new
MissingServiceException("Service matching '$type' type not found.");}elseif(count($found)>1){throw
new
AmbiguousServiceException("Found more than one service ('".implode("', '",$found)."') matching '$type' type.");}return$this->getService($found[0]);}function
getServiceNamesByTag($tag){$found=array();foreach($this->registry
as$name=>$service){if(isset($this->tags[$name][$tag])){$found[$name]=$this->tags[$name][$tag];}}return$found;}function
hasService($name){return
isset($this->registry[$name])||isset($this->factories[$name])||method_exists($this,"createService$name");}function
checkServiceType($name,$type){return
isset($this->tags[$name][self::TAG_TYPEHINT][0])?!strcasecmp($this->tags[$name][self::TAG_TYPEHINT][0],$type):(isset($this->registry[$name])&&$this->registry[$name]instanceof$type);}function
expand($s){if(is_string($s)&&strpos($s,'%')!==FALSE){$that=$this;return@preg_replace_callback('#%([a-z0-9._-]*)%#i',function($m)use($that){list(,$param)=$m;if($param===''){return'%';}elseif(!isset($that->params[$param])){throw
new
NetteX\InvalidArgumentException("Missing parameter '$param'.");}elseif(!is_scalar($val=$that->params[$param])){throw
new
NetteX\InvalidStateException("Parameter '$param' is not scalar.");}return$val;},$s);}return$s;}function&__get($name){if(!isset($this->registry[$name])){$this->getService($name);}return$this->registry[$name];}function
__set($name,$value){$this->addService($name,$value);}function
__isset($name){return$this->hasService($name);}function
__unset($name){$this->removeService($name);}}class
ContainerBuilder
extends
NetteX\Object{function
addDefinitions(IContainer$container,array$definitions){foreach($definitions
as$name=>$definition){if(!is_array($definition)){$definition=array('class'=>$definition);}$arguments=isset($definition['arguments'])?$definition['arguments']:array();$expander=function(&$val)use($container){$val=$val[0]==='@'?$container->getService(substr($val,1)):$container->expand($val);};if(isset($definition['class'])){$class=$definition['class'];$methods=isset($definition['methods'])?$definition['methods']:array();$factory=function($container)use($class,$arguments,$methods,$expander){$class=$container->expand($class);if($arguments){array_walk_recursive($arguments,$expander);$service=NetteX\Reflection\ClassType::from($class)->newInstanceArgs($arguments);}else{$service=new$class;}array_walk_recursive($methods,$expander);foreach($methods
as$method){call_user_func_array(array($service,$method[0]),isset($method[1])?$method[1]:array());}return$service;};}elseif(isset($definition['factory'])){array_unshift($arguments,$definition['factory']);$factory=function($container)use($arguments,$expander){array_walk_recursive($arguments,$expander);$factory=$arguments[0];$arguments[0]=$container;return
call_user_func_array($factory,$arguments);};}else{throw
new
NetteX\InvalidStateException("The definition of service '$name' is missing factory method.");}if(isset($definition['tags'])){$tags=(array)$definition['tags'];array_walk_recursive($tags,$expander);}else{$tags=NULL;}$container->addService($name,$factory,$tags);}}function
generateCode(array$definitions){$code='';foreach($definitions
as$name=>$definition){$name=$this->varExport($name);if(is_scalar($definition)){$factory=$this->varExport($definition);$code.="\$container->addService($name, $factory);\n\n";continue;}$arguments=$this->argsExport(isset($definition['arguments'])?$definition['arguments']:array());if(isset($definition['class'])){$class=$this->argsExport(array($definition['class']));$methods=isset($definition['methods'])?$definition['methods']:array();$factory="function(\$container) {\n\t\$class = $class; \$service = new \$class($arguments);\n";foreach($methods
as$method){$args=isset($method[1])?$this->argsExport($method[1]):'';$factory.="\t\$service->$method[0]($args);\n";}$factory.="\treturn \$service;\n}";}elseif(isset($definition['factory'])){$factory=$this->argsExport(array($definition['factory']));$factory="function(\$container) {\n\treturn call_user_func(\n\t\t$factory,\n\t\t\$container".($arguments?",\n\t\t$arguments":'')."\n\t);\n}";}else{throw
new
NetteX\InvalidStateException("The definition of service '$name' is missing factory method.");}$tags=isset($definition['tags'])?$this->argsExport(array($definition['tags'])):'NULL';$code.="\$container->addService($name, $factory, $tags);\n\n";}return$code;}private
function
argsExport($args){$args=implode(', ',array_map(array($this,'varExport'),$args));$args=preg_replace("#'@(\w+)'#",'\$container->getService(\'$1\')',$args);$args=preg_replace("#('[^']*%[^']*')#",'\$container->expand($1)',$args);return$args;}private
function
varExport($arg){return
preg_replace('#\n *#',' ',var_export($arg,TRUE));}}class
MissingServiceException
extends
NetteX\InvalidStateException{}class
...
NetteX\Object
implements
IServiceBuilder{private$class;function
__construct($class){$this->class=$class;}function
getClass(){return$this->class;}function
createService(NetteX\DI\IContainer$container){if(!class_exists($this->class)){throw
new
NetteX\InvalidStateException("Cannot instantiate service, class '$this->class' not found.");}return
new$this->class;}}}namespace NetteX\Diagnostics{use
NetteX;class
Bar
extends
NetteX\Object{private$panels=array();function
addPanel(IBarPanel$panel,$id=NULL){if($id===NULL){$c=0;do{$id=get_class($panel).($c++?"-$c":'');}while(isset($this->panels[$id]));}$this->panels[$id]=$panel;}function
...
json_encode(NetteX\Utils\Strings::fixEncoding($output))?>;
...
extends
NetteX\Object{private$panels=array();function
addPanel($panel,$id=NULL){if($id===NULL){$this->panels[]=$panel;}else{$this->panels[$id]=$panel;}}function
...
self::highlightFile($ex->getFile(),$ex->getLine())?></pre><?php endif?>
			</div></div>
...
NetteX\Utils\Html::el('a')->href(strtr(Debugger::$editor,array('%file'=>rawurlencode($file),'%line'=>$line)))->title("$file:$line")->setHtml(htmlSpecialChars(rtrim($dir,DIRECTORY_SEPARATOR)).DIRECTORY_SEPARATOR.'<b>'.htmlSpecialChars(basename($file)).'</b>');}else{return
NetteX\Utils\Html::el('span')->setText($file);}}static
function
...
function
split($subject,$pattern,$flags=0){NetteX\Diagnostics\Debugger::tryError();$res=preg_split($pattern,$subject,-1,$flags|PREG_SPLIT_DELIM_CAPTURE);self::catchPregError($pattern);return$res;}static
function
match($subject,$pattern,$flags=0,$offset=0){NetteX\Diagnostics\Debugger::tryError();$res=preg_match($pattern,$subject,$m,$flags,$offset);self::catchPregError($pattern);if($res){return$m;}}static
function
matchAll($subject,$pattern,$flags=0,$offset=0){NetteX\Diagnostics\Debugger::tryError();$res=preg_match_all($pattern,$subject,$m,($flags&PREG_PATTERN_ORDER)?$flags:($flags|PREG_SET_ORDER),$offset);self::catchPregError($pattern);return$m;}static
function
replace($subject,$pattern,$replacement=NULL,$limit=-1){NetteX\Diagnostics\Debugger::tryError();if(is_object($replacement)||is_array($replacement)){if($replacement
instanceof
NetteX\Callback){$replacement=$replacement->getNative();}if(!is_callable($replacement,FALSE,$textual)){NetteX\Diagnostics\Debugger::catchError($foo);throw
new
NetteX\InvalidStateException("Callback '$textual' is not callable.");}$res=preg_replace_callback($pattern,$replacement,$subject,$limit);if(NetteX\Diagnostics\Debugger::catchError($e)){$trace=$e->getTrace();if(isset($trace[2]['class'])&&$trace[2]['class']===__CLASS__){throw
new
...
function
catchPregError($pattern){if(NetteX\Diagnostics\Debugger::catchError($e)){throw
new
...
RegexpException
extends\Exception{}}namespace NetteX\Diagnostics{use
NetteX;class
Logger
extends
NetteX\Object{const
...
FALSE;}$item=array('name'=>'PHP','level'=>$priority,'order'=>count(self::$payload['logs']),'time'=>str_pad(number_format((microtime(TRUE)-Debugger::$time)*1000,1,'.',' '),8,'0',STR_PAD_LEFT).' ms','template'=>'','message'=>'','style'=>'background:#767ab6');$args=func_get_args();if(isset($args[0])&&is_string($args[0])){$item['template']=array_shift($args);}if(isset($args[0])&&$args[0]instanceof\Exception){$e=array_shift($args);$trace=$e->getTrace();if(isset($trace[0]['class'])&&$trace[0]['class']==='NetteX\Diagnostics\Debugger'&&($trace[0]['function']==='_shutdownHandler'||$trace[0]['function']==='_errorHandler')){unset($trace[0]);}$file=str_replace(dirname(dirname(dirname($e->getFile()))),"\xE2\x80\xA6",$e->getFile());$item['template']=($e
instanceof\ErrorException?'':get_class($e).': ').$e->getMessage().($e->getCode()?' #'.$e->getCode():'').' in '.$file.':'.$e->getLine();$item['pathname']=$e->getFile();$item['lineno']=$e->getLine();}else{$trace=debug_backtrace();if(isset($trace[1]['class'])&&$trace[1]['class']==='NetteX\Diagnostics\Debugger'&&($trace[1]['function']==='fireLog')){unset($trace[0]);}foreach($trace
...
NetteX\Utils\Strings::fixEncoding($var);}elseif(is_array($var)){static$marker;if($marker===NULL){$marker=uniqid("\x00",TRUE);}if(isset($var[$marker])){return"\xE2\x80\xA6RECURSION\xE2\x80\xA6";}elseif($level<Debugger::$maxDepth||!Debugger::$maxDepth){$var[$marker]=TRUE;$res=array();foreach($var
...
<?php }return
ob_get_clean();}}}namespace NetteX\Forms{use
NetteX;class
ControlGroup
extends
...
loadHttpData(){$path=explode('[',strtr(str_replace(array('[]',']'),'',$this->getHtmlName()),'.','_'));$this->setValue(NetteX\Utils\Arrays::get($this->getForm()->getHttpData(),$path));}function
...
loadHttpData(){$path=$this->getHtmlName();$path=explode('[',strtr(str_replace(']','',strpos($path,'[')===FALSE?$path.'.x':substr($path,0,-2)),'.','_'));$this->setValue(NetteX\Utils\Arrays::get($this->getForm()->getHttpData(),$path)!==NULL);}}class
SelectBox
extends
...
addFilter($filter){$this->filters[]=callbackX($filter);return$this;}function
...
addRule($operation,$message=NULL,$arg=NULL){if($operation===Form::FLOAT){$this->addFilter(callbackX(__CLASS__,'filterFloat'));}return
...
Strings::length($control->getValue())<=$length;}static
function
validateLength(TextBase$control,$range){if(!is_array($range)){$range=array($range,$range);}$len=Strings::length($control->getValue());return($range[0]===NULL||$len>=$range[0])&&($range[1]===NULL||$len<=$range[1]);}static
function
validateEmail(TextBase$control){$atom="[-a-z0-9!#$%&'*+/=?^_`{|}~]";$localPart="(?:\"(?:[ !\\x23-\\x5B\\x5D-\\x7E]*|\\\\[ -~])+\"|$atom+(?:\\.$atom+)*)";$chars="a-z0-9\x80-\xFF";$domain="[$chars](?:[-$chars]{0,61}[$chars])";return(bool)Strings::match($control->getValue(),"(^$localPart@(?:$domain?\\.)+[-$chars]{2,19}\\z)i");}static
function
validateUrl(TextBase$control){$chars="a-z0-9\x80-\xFF";return(bool)Strings::match($control->getValue(),"#^(?:https?://|)(?:[$chars](?:[-$chars]{0,61}[$chars])?\\.)+[-$chars]{2,19}(/\S*)?$#i");}static
function
...
function
validateInteger(TextBase$control){return(bool)Strings::match($control->getValue(),'/^-?[0-9]+$/');}static
function
validateFloat(TextBase$control){return(bool)Strings::match($control->getValue(),'/^-?[0-9]*[.,]?[0-9]+$/');}static
function
...
extends
TextBase{function
__construct($label=NULL,$cols=NULL,$maxLength=NULL){parent::__construct($label);$this->control->type='text';$this->control->size=$cols;$this->control->maxlength=$maxLength;$this->filters[]=callbackX($this,'sanitize');$this->value='';}function
sanitize($value){if($this->control->maxlength&&NetteX\Utils\Strings::length($value)>$this->control->maxlength){$value=iconv_substr($value,0,$this->control->maxlength,'UTF-8');}return
NetteX\Utils\Strings::trim(strtr($value,"\r\n",'  '));}function
...
callbackX(get_class($rule->control),self::VALIDATE_PREFIX.ltrim($op,':'));}else{return
callbackX($op);}}static
function
...
Context
extends
NetteX\Object{private$request;private$response;function
...
NetteX\Utils\Arrays::get($this->files,$args);}final
function
...
NetteX\Utils\Strings::random(2e3," \t\r\n");self::$fixIE=FALSE;}}function
...
new
NetteX\InvalidStateException('session_start(): '.$e->getMessage(),0,$e);}self::$started=TRUE;if($this->regenerationNeeded){session_regenerate_id(TRUE);$this->regenerationNeeded=FALSE;}unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];if(empty($nf)){$nf=array('C'=>0);}else{$nf['C']++;}$browserKey=$this->request->getCookie('nette-browser');if(!$browserKey){$browserKey=NetteX\Utils\Strings::random();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$namespace=>$metadata){if(is_array($metadata)){foreach($metadata
...
new
NetteX\InvalidStateException('Session is not started.');}session_destroy();$_SESSION=NULL;self::$started=FALSE;if(!$this->response->isSent()){$params=session_get_cookie_params();$this->response->deleteCookie(session_name(),$params['path'],$params['domain'],$params['secure']);}}function
exists(){return
...
getNamespace($name,$class='NetteX\Http\SessionNamespace'){return
new$class($this,$name);}function
hasNamespace($name){if($this->exists()&&!self::$started){$this->start();}return!empty($_SESSION['__NF']['DATA'][$name]);}function
...
function
sendCookie(){$cookie=$this->getCookieParams();$this->response->setCookie(session_name(),session_id(),$cookie['lifetime']?$cookie['lifetime']+time():0,$cookie['path'],$cookie['domain'],$cookie['secure'],$cookie['httponly'])->setCookie('nette-browser',$_SESSION['__NF']['B'],Response::BROWSER,$cookie['path'],$cookie['domain']);}}final
class
SessionNamespace
extends
NetteX\Object
...
new
NetteX\InvalidArgumentException("Session namespace must be a string, ".gettype($name)." given.");}$this->session=$session;$this->name=$name;}private
function
...
new
NetteX\InvalidArgumentException("Malformed or unsupported URI '$url'.");}foreach($parts
...
implements
IUser{const
MANUAL=1,INACTIVITY=2,BROWSER_CLOSED=3;public$guestRole='guest';public$authenticatedRole='authenticated';public$onLoggedIn;public$onLoggedOut;private$namespace='';private$session;private$context;function
__construct(NetteX\DI\IContainer$context){$this->context=$context;}function
login($username=NULL,$password=NULL){$this->logout(TRUE);$credentials=func_get_args();$this->setIdentity($this->context->authenticator->authenticate($credentials));$this->setAuthenticated(TRUE);$this->onLoggedIn($this);}final
function
...
setAuthenticationHandler(IAuthenticator$handler){$this->context->authenticator=$handler;return$this;}final
function
getAuthenticationHandler(){return$this->context->authenticator;}function
...
function
getSessionNamespace($need){if($this->session!==NULL){return$this->session;}if(!$need&&!$this->context->session->exists()){return
NULL;}$this->session=$session=$this->context->session->getNamespace('NetteX.Web.User/'.$this->namespace);if(!$session->identity
instanceof
...
function
setAuthenticated($state){$session=$this->getSessionNamespace(TRUE);$session->authenticated=(bool)$state;$this->context->session->regenerateId();if($state){$session->reason=NULL;$session->authTime=time();}else{$session->reason=self::MANUAL;$session->authTime=NULL;}return$this;}protected
function
...
isAllowed($resource=IAuthorizator::ALL,$privilege=IAuthorizator::ALL){$authorizator=$this->context->authorizator;foreach($this->getRoles()as$role){if($authorizator->isAllowed($role,$resource,$privilege)){return
TRUE;}}return
FALSE;}function
setAuthorizationHandler(IAuthorizator$handler){$this->context->authorizator=$handler;return$this;}final
function
getAuthorizationHandler(){return$this->context->authorizator;}}}namespace NetteX\Iterators{use
NetteX;class
CachingIterator
...
Engine
extends
NetteX\Object{public$parser;function
__construct(){$this->parser=new
Parser;Macros\CoreMacros::install($this->parser);$this->parser->addMacro('cache',new
Macros\CacheMacro($this->parser));Macros\UIMacros::install($this->parser);Macros\FormMacros::install($this->parser);}function
__invoke($s){$this->parser->context=Parser::CONTEXT_TEXT;$this->parser->escape='NetteX\Templating\DefaultHelpers::escapeHtml|';$this->parser->setDelimiters('\\{(?![\\s\'"{}*])','\\}');return$this->parser->parse($s);}}class
HtmlNode
extends
NetteX\Object{public$name;public$isEmpty=FALSE;public$attrs=array();public$closing=FALSE;public$offset;function
__construct($name){$this->name=$name;$this->isEmpty=isset(NetteX\Utils\Html::$emptyElements[strtolower($this->name)]);}}class
MacroNode
extends
NetteX\Object{public$macro;public$name;public$isEmpty=FALSE;public$args;public$modifiers;public$closing=FALSE;public$tokenizer;public$offset;public$parentNode;public$content;public$data;function
__construct(IMacro$macro,$name,$args=NULL,$modifiers=NULL,MacroNode$parentNode=NULL){$this->macro=$macro;$this->name=(string)$name;$this->modifiers=(string)$modifiers;$this->parentNode=$parentNode;$this->tokenizer=new
...
close($content){$this->closing=TRUE;$this->content=$content;return$this->macro->nodeClosed($this);}}}namespace NetteX\Latte\Macros{use
NetteX;use
NetteX\Latte;class
CacheMacro
extends
NetteX\Object
implements
Latte\IMacro{private$parser;private$used;function
__construct(Latte\Parser$parser){$this->parser=$parser;}function
initialize(){$this->used=FALSE;}function
finalize(){if($this->used){return
array('NetteX\Latte\Macros\CacheMacro::initRuntime($template, $_g);');}}function
nodeOpened(Latte\MacroNode$node){$this->used=TRUE;$node->isEmpty=FALSE;return
Latte\PhpWriter::using($node)->write('<?php if (NetteX\Latte\Macros\CacheMacro::createCache($netteCacheStorage, %var, $_g->caches, %node.array?)) { ?>',NetteX\Utils\Strings::random());}function
nodeClosed(Latte\MacroNode$node){return'<?php $_l->tmp = array_pop($_g->caches); if (!$_l->tmp instanceof \stdClass) $_l->tmp->end(); } ?>';}static
function
initRuntime($template,$global){if(!empty($global->caches)){end($global->caches)->dependencies[NetteX\Caching\Cache::FILES][]=$template->getFile();}}static
function
createCache(NetteX\Caching\IStorage$cacheStorage,$key,&$parents,$args=NULL){if($args){if(array_key_exists('if',$args)&&!$args['if']){return$parents[]=(object)NULL;}$key=array_merge(array($key),array_intersect_key($args,range(0,count($args))));}if($parents){end($parents)->dependencies[NetteX\Caching\Cache::ITEMS][]=$key;}$cache=new
NetteX\Caching\Cache($cacheStorage,'NetteX.Templating.Cache');if($helper=$cache->start($key)){$helper->dependencies=array(NetteX\Caching\Cache::TAGS=>isset($args['tags'])?$args['tags']:NULL,NetteX\Caching\Cache::EXPIRATION=>isset($args['expire'])?$args['expire']:'+ 7 days');$parents[]=$helper;}return$helper;}}use
NetteX\Latte\MacroNode;class
MacroSet
extends
NetteX\Object
implements
Latte\IMacro{public$parser;private$macros;function
__construct(Latte\Parser$parser){$this->parser=$parser;}function
addMacro($name,$begin,$end=NULL){$this->macros[$name]=array($begin,$end);$this->parser->addMacro($name,$this);}function
initialize(){}function
finalize(){}function
...
new
ParseException('Missing condition in {if} macro.');}return$writer->write('if (%node.args) ob_end_flush(); else ob_end_clean()');}return'endif';}function
macroTranslate(MacroNode$node,$writer){return$writer->write('echo %modify','$template->translate('.$writer->formatArgs().')');}function
macroSyntax(MacroNode$node){if($node->closing){$node->args='latte';}switch($node->args){case'':case'latte':$this->parser->setDelimiters('\\{(?![\\s\'"{}])','\\}');break;case'double':$this->parser->setDelimiters('\\{\\{(?![\\s\'"{}])','\\}\\}');break;case'asp':$this->parser->setDelimiters('<%\s*','\s*%>');break;case'python':$this->parser->setDelimiters('\\{[{%]\s*','\s*[%}]\\}');break;case'off':$this->parser->setDelimiters('[^\x00-\xFF]','');break;default:throw
new
ParseException("Unknown syntax '$node->args'");}}function
macroInclude(MacroNode$node,$writer){$code=$writer->write('NetteX\Latte\Macros\CoreMacros::includeTemplate(%node.word, %node.array? + $template->getParams(), $_l->templates[%var])',$this->parser->templateId);if($node->modifiers){return$writer->write('echo %modify',$code.'->__toString(TRUE)');}else{return$code.'->render()';}}function
...
macroForeach(MacroNode$node,$writer){return'$iterations = 0; foreach ($iterator = $_l->its[] = new NetteX\Iterators\CachingIterator('.preg_replace('#(.*)\s+as\s+#i','$1) as ',$writer->formatArgs(),1).'):';}function
macroClass(MacroNode$node,$writer){return$writer->write('if ($_l->tmp = trim(implode(" ", array_unique(%node.array)))) echo \' class="\' . %escape($_l->tmp) . \'"\'');}function
macroAttr(MacroNode$node,$writer){return$writer->write('if (($_l->tmp = (string) (%node.args)) !== \'\') echo \' @@="\' . %escape($_l->tmp) . \'"\'');}function
macroOldAttr(MacroNode$node){return
NetteX\Utils\Strings::replace($node->args.' ','#\)\s+#',')->');}function
macroDump(MacroNode$node,$writer){$args=$writer->formatArgs();return$writer->write('NetteX\Diagnostics\Debugger::barDump('.($node->args?"array(%var => $args)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))',$args);}function
...
macroVar(MacroNode$node,$writer){$out='';$var=TRUE;$tokenizer=$writer->preprocess();while($token=$tokenizer->fetchToken()){if($var&&($token['type']===Latte\MacroTokenizer::T_SYMBOL||$token['type']===Latte\MacroTokenizer::T_VARIABLE)){if($node->name==='default'){$out.="'".ltrim($token['value'],"$")."'";}else{$out.='$'.ltrim($token['value'],"$");}}elseif(($token['value']==='='||$token['value']==='=>')&&$token['depth']===0){$out.=$node->name==='default'?'=>':'=';$var=FALSE;}elseif($token['value']===','&&$token['depth']===0){$out.=$node->name==='default'?',':';';$var=TRUE;}else{$out.=$writer->canQuote($tokenizer)?"'$token[value]'":$token['value'];}}return$node->name==='default'?"extract(array($out), EXTR_SKIP)":$out;}function
macroExpr(MacroNode$node,$writer){return$writer->write(($node->name==='?'?'':'echo ').'%modify',$writer->formatArgs());}static
function
...
function
initRuntime($template,$templateId){if(isset($template->_l)){$local=$template->_l;unset($template->_l);}else{$local=(object)NULL;}$local->templates[$templateId]=$template;if(!isset($template->_g)){$template->_g=(object)NULL;}return
...
function
install(Latte\Parser$parser){$me=new
static($parser);$me->addMacro('form','$form = $control[%node.word]; echo $form->getElementPrototype()->addAttributes(%node.array)->startTag()','echo $form->getElementPrototype()->endTag()');$me->addMacro('label',array($me,'macroLabel'),'?></label><?php');$me->addMacro('input','echo $form[%node.word]->getControl()->addAttributes(%node.array)');}function
macroLabel(MacroNode$node,$writer){$cmd='if ($_label = $form[%node.word]->getLabel()) echo $_label->addAttributes(%node.array)';if($node->isEmpty=(substr($node->args,-1)==='/')){$node->setArgs(substr($node->args,0,-1));return$writer->write($cmd);}else{return$writer->write($cmd.'->startTag()');}}}class
UIMacros
extends
MacroSet{const
...
if ($_l->extends) {
	...
	return NetteX\Latte\Macros\UIMacros::renderSnippets($control, $_l, get_defined_vars());
}';$epilog[]='
// template extending support
if ($_l->extends) {
	ob_end_clean();
	NetteX\Latte\Macros\CoreMacros::includeTemplate($_l->extends, get_defined_vars(), $template)->render();
}';}else{$prolog[]='
// snippets support
...
macroBlock(MacroNode$node,$writer){$name=$node->tokenizer->fetchWord();if($node->name==='block'&&$name===FALSE){return$node->modifiers===''?'':'ob_start()';}else{$node->data->name=$name=($node->name==='snippet'?'_':'').ltrim($name,'#');if($name==NULL){throw
new
ParseException("Missing block name.");}if($name[0]==='$'){$func='_lb'.substr(md5($this->parser->templateId.$name),0,10).'_'.preg_replace('#[^a-z0-9_]#i','_',$name);return"//\n// block $name\n//\n"."if (!function_exists(\$_l->blocks[$name][] = '$func')) { "."function $func(\$_l, \$_args) { ".(PHP_VERSION_ID>50208?'extract($_args)':'foreach ($_args as $__k => $__v) $$__k = $__v');}if(isset($this->namedBlocks[$name])){throw
new
ParseException("Cannot redeclare static block '$name'");}$top=empty($node->parentNode);$this->namedBlocks[$name]=TRUE;$include='call_user_func(reset($_l->blocks[%var]), $_l, '.($node->name==='snippet'?'$template->getParams()':'get_defined_vars()').')';if($node->modifiers){$include="ob_start(); $include; echo %modify";}if($node->name==='snippet'){$tag=$node->tokenizer->fetchWord();$tag=trim($tag,'<>');$tag=$tag?$tag:'div';return$writer->write("?><$tag id=\"<?php echo \$control->getSnippetId(%var) ?>\"><?php $include ?></$tag><?php ",(string)substr($name,1),$name,'ob_get_clean()');}elseif($node->name==='define'){return'';}elseif(!$top){return$writer->write($include,$name,'ob_get_clean()');}elseif($this->extends){return'';}else{return$writer->write("if (!\$_l->extends) { $include; }",$name,'ob_get_clean()');}}}function
macroBlockEnd(MacroNode$node,$writer){if($node->name==='capture'){return$this->macroCaptureEnd($node,$writer);}elseif(isset($node->data->name)){if($node->data->name[0]==='$'){return$writer->write("}} call_user_func(reset(\$_l->blocks[{$node->data->name}]), \$_l, get_defined_vars())");}$this->namedBlocks[$node->data->name]=$node->content;return$node->content='';}elseif($node->modifiers){return$writer->write('echo %modify','ob_get_clean()');}}function
macroIfset(MacroNode$node,$writer){if(strpos($node->args,'#')===FALSE){return
...
macroLink(MacroNode$node,$writer){return$writer->write('echo %escape('.($node->name==='plink'?'$presenter':'$control').'->link(%node.word, %node.array?))');}function
macroIfCurrent(MacroNode$node,$writer){return$writer->write(($node->args?'try { $presenter->link(%node.word, %node.array?); } catch (NetteX\Application\UI\InvalidLinkException $e) {}':'').'; if ($presenter->getLastCreatedRequestFlag("current")):');}function
macroContentType(MacroNode$node,$writer){if(strpos($node->args,'html')!==FALSE){$this->parser->escape='NetteX\Templating\DefaultHelpers::escapeHtml|';$this->parser->context=Latte\Parser::CONTEXT_TEXT;}elseif(strpos($node->args,'xml')!==FALSE){$this->parser->escape='NetteX\Templating\DefaultHelpers::escapeXml';$this->parser->context=Latte\Parser::CONTEXT_NONE;}elseif(strpos($node->args,'javascript')!==FALSE){$this->parser->escape='NetteX\Templating\DefaultHelpers::escapeJs';$this->parser->context=Latte\Parser::CONTEXT_NONE;}elseif(strpos($node->args,'css')!==FALSE){$this->parser->escape='NetteX\Templating\DefaultHelpers::escapeCss';$this->parser->context=Latte\Parser::CONTEXT_NONE;}elseif(strpos($node->args,'plain')!==FALSE){$this->parser->escape='';$this->parser->context=Latte\Parser::CONTEXT_NONE;}else{$this->parser->escape='$template->escape';$this->parser->context=Latte\Parser::CONTEXT_NONE;}if(strpos($node->args,'/')){return$writer->write('$netteHttpResponse->setHeader("Content-Type", %node.word)');}}static
function
...
instanceof
NetteX\Application\UI\Control){foreach($control->getComponents(FALSE,'NetteX\Application\UI\Control')as$child){if($child->isControlInvalid()){$child->render();}}}}}}namespace NetteX\Utils{use
NetteX;use
NetteX\Utils\Strings;class
Tokenizer
extends
NetteX\Object{public$tokens;public$position=0;public$ignored=array();private$input;private$re;private$types;public$current;function
...
isCurrent($arg){if(is_array($this->current)){return
...
TokenizerException
extends\Exception{}}namespace NetteX\Latte{use
NetteX;class
MacroTokenizer
extends
NetteX\Utils\Tokenizer{const
T_WHITESPACE=1,T_COMMENT=2,T_SYMBOL=3,T_NUMBER=4,T_VARIABLE=5,T_STRING=6,T_CAST=7,T_KEYWORD=8,T_CHAR=9;function
__construct($input){parent::__construct(array(self::T_WHITESPACE=>'\s+',self::T_COMMENT=>'(?s)/\*.*?\*/',self::T_STRING=>Parser::RE_STRING,self::T_KEYWORD=>'(?:true|false|null|and|or|xor|clone|new|instanceof|return|continue|break|[A-Z_][A-Z0-9_]{2,})(?![\d\pL_])',self::T_CAST=>'\([a-z]+\)',self::T_VARIABLE=>'\$[\d\pL_]+',self::T_NUMBER=>'[+-]?[0-9]+(?:\.[0-9]+)?(?:e[0-9]+)?',self::T_SYMBOL=>'[\d\pL_]+(?:-[\d\pL_]+)*',self::T_CHAR=>'::|=>|[^"\']'),'u');$this->ignored=array(self::T_COMMENT,self::T_WHITESPACE);$this->tokenize($input);}function
fetchWord(){$word=$this->fetchUntil(self::T_WHITESPACE,',');$this->fetch(',');$this->fetchAll(self::T_WHITESPACE,self::T_COMMENT);return$word;}}}namespace NetteX\Templating{use
NetteX;class
FilterException
extends
NetteX\InvalidStateException{public$sourceFile;public$sourceLine;function
__construct($message,$code=0,$sourceLine=0){$this->sourceLine=(int)$sourceLine;parent::__construct($message,$code);}function
setSourceFile($file){$this->sourceFile=(string)$file;$this->message=rtrim($this->message,'.')." in ".str_replace(dirname(dirname($file)),'...',$file).($this->sourceLine?":$this->sourceLine":'');}}}namespace NetteX\Latte{use
NetteX;class
ParseException
extends
NetteX\Templating\FilterException{}use
NetteX\Utils\Strings;class
Parser
extends
NetteX\Object{const
RE_STRING='\'(?:\\\\.|[^\'\\\\])*\'|"(?:\\\\.|[^"\\\\])*"';const
N_PREFIX='n:';private$macroRe;private$input;private$output;private$offset;private$quote;private$macros;private$macroHandlers;private$htmlNodes=array();private$macroNodes=array();public$context=Parser::CONTEXT_NONE;public$escape;public$templateId;const
...
contextText(){$matches=$this->match('~
		...
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['htmlcomment'])){$this->context=self::CONTEXT_COMMENT;$this->escape='NetteX\Templating\DefaultHelpers::escapeHtmlComment';}elseif(empty($matches['closing'])){$this->htmlNodes[]=$node=new
HtmlNode($matches['tag']);$node->offset=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='NetteX\Templating\DefaultHelpers::escapeHtml';}else{do{$node=array_pop($this->htmlNodes);if(!$node){$node=new
HtmlNode($matches['tag']);}}while(strcasecmp($node->name,$matches['tag']));$this->htmlNodes[]=$node;$node->closing=TRUE;$node->offset=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='NetteX\Templating\DefaultHelpers::escapeHtml';}return$matches;}private
function
contextCData(){$node=end($this->htmlNodes);$matches=$this->match('~
			</'.$node->name.'(?![a-z0-9:])| ##  end HTML tag </tag
			'.$this->macroRe.'           ##  curly tag
		~xsi');if($matches&&empty($matches['macro'])&&empty($matches['comment'])){$node->closing=TRUE;$node->offset=strlen($this->output);$this->context=self::CONTEXT_TAG;$this->escape='NetteX\Templating\DefaultHelpers::escapeHtml';}return$matches;}private
function
contextTag(){$matches=$this->match('~
			(?P<end>\ ?/?>)(?P<tagnewline>[ \t]*\n)?|  ##  end of HTML tag
			'.$this->macroRe.'|          ##  curly tag
			\s*(?P<attr>[^\s/>={]+)(?:\s*=\s*(?P<value>["\']|[^\s/>{]+))? ## begin of HTML attribute
		~xsi');if(!$matches||!empty($matches['macro'])||!empty($matches['comment'])){}elseif(!empty($matches['end'])){$node=end($this->htmlNodes);$isEmpty=!$node->closing&&(strpos($matches['end'],'/')!==FALSE||$node->isEmpty);if($isEmpty){$matches[0]=(NetteX\Utils\Html::$xhtml?' />':'>').(isset($matches['tagnewline'])?$matches['tagnewline']:'');}if(!empty($node->attrs)){$code=substr($this->output,$node->offset).$matches[0];$this->output=substr($this->output,0,$node->offset);$this->writeAttrsMacro($code,$node->attrs,$node->closing);if($isEmpty){$this->writeAttrsMacro('',$node->attrs,TRUE);}$matches[0]='';}if($isEmpty){$node->closing=TRUE;}if(!$node->closing&&(strcasecmp($node->name,'script')===0||strcasecmp($node->name,'style')===0)){$this->context=self::CONTEXT_CDATA;$this->escape='NetteX\Templating\DefaultHelpers::escape'.(strcasecmp($node->name,'style')?'Js':'Css');}else{$this->context=self::CONTEXT_TEXT;$this->escape='NetteX\Templating\DefaultHelpers::escapeHtml|';if($node->closing){array_pop($this->htmlNodes);}}}else{$name=$matches['attr'];$value=isset($matches['value'])?$matches['value']:'';$node=end($this->htmlNodes);if(Strings::startsWith($name,self::N_PREFIX)){$name=substr($name,strlen(self::N_PREFIX));if($value==='"'||$value==="'"){if($matches=$this->match('~(.*?)'.$value.'~xsi')){$value=$matches[1];}}$node->attrs[$name]=$value;$matches[0]='';}elseif($value==='"'||$value==="'"){$this->context=self::CONTEXT_ATTRIBUTE;$this->quote=$value;$this->escape=strncasecmp($name,'on',2)?('NetteX\Templating\DefaultHelpers::escapeHtml'.(strcasecmp($name,'style')?"|$value":'Css')):"NetteX\\Templating\\DefaultHelpers::escapeHtmlJs|$value";}}return$matches;}private
function
...
function
match($re){if($matches=Strings::match($this->input,$re,PREG_OFFSET_CAPTURE,$this->offset)){$this->output.=substr($this->input,$this->offset,$matches[0][1]-$this->offset);$this->offset=$matches[0][1]+strlen($matches[0][0]);foreach($matches
as$k=>$v)$matches[$k]=$v[0];}return$matches;}function
getLine(){return$this->input&&$this->offset?substr_count($this->input,"\n",0,$this->offset)+1:NULL;}function
...
new
ParseException("Unknown macro-attribute ".self::N_PREFIX.implode(' and '.self::N_PREFIX,array_keys($attrs)),0,$this->line);}foreach($left
as$item){$this->writeMacro($item[0],$item[1]);if(substr($this->output,-2)==='?>'){$this->output.="\n";}}$this->output.=$code;foreach($right
as$item){$this->writeMacro($item[0],$item[1]);if(substr($this->output,-2)==='?>'){$this->output.="\n";}}}function
expandMacro($name,$args,$modifiers=NULL){if(empty($this->macros[$name])){throw
new
ParseException("Unknown macro {{$name}}",0,$this->line);}foreach(array_reverse($this->macros[$name])as$macro){$node=new
...
new
ParseException("Unhandled macro {{$name}}",0,$this->line);}function
parseMacro($macro){$match=Strings::match($macro,'~^
			(
		...
		()$~isx');if(!$match){return
FALSE;}if($match['name']===''){$match['name']=$match['shortname']?:'=';if(!$match['noescape']){$match['modifiers'].='|escape';}}return
array($match['name'],trim($match['args']),$match['modifiers']);}}class
PhpWriter
extends
NetteX\Object{private$argsTokenizer;private$modifiers;private$escape;static
function
using(MacroNode$node,$escape=NULL){return
new
static($node->tokenizer,$node->modifiers,$escape);}function
__construct(MacroTokenizer$argsTokenizer,$modifiers=NULL,$escape=NULL){$this->argsTokenizer=$argsTokenizer;$this->modifiers=$modifiers;$this->escape=explode('|',$escape);}function
write($mask){$args=func_get_args();array_shift($args);$word=strpos($mask,'%node.word')===FALSE?NULL:$this->argsTokenizer->fetchWord();$mask=str_replace('%escape',$this->escape[0],$mask);$me=$this;return
NetteX\Utils\Strings::replace($mask,'#([,+]\s*)?%(node\.word|node\.array|node\.args|modify|var)(\?)?(\s*\+\s*)?()#',function($m)use($me,$word,&$args){list(,$l,$macro,$cond,$r)=$m;switch($macro){case'node.word':$code=$me->formatWord($word);break;case'node.args':$code=$me->formatArgs();break;case'node.array':$code=$me->formatArray();$code=$cond&&$code==='array()'?'':$code;break;case'modify':$code=$me->formatModifiers(array_shift($args));break;case'var':$code=var_export(array_shift($args),TRUE);break;}if($cond&&$code===''){return$r?$l:$r;}else{return$l.$code.$r;}});}function
formatModifiers($var){$modifiers=ltrim($this->modifiers,'|');if(!$modifiers){return$var;}$tokenizer=$this->preprocess(new
MacroTokenizer($modifiers));$inside=FALSE;while($token=$tokenizer->fetchToken()){if($token['type']===MacroTokenizer::T_WHITESPACE){$var=rtrim($var).' ';}elseif(!$inside){if($token['type']===MacroTokenizer::T_SYMBOL){if($this->escape&&$token['value']==='escape'){$var=$this->escape[0]."($var".(isset($this->escape[1])?', '.var_export($this->escape[1],TRUE):'');}else{$var="\$template->".$token['value']."($var";}$inside=TRUE;}else{throw
new
...
register(){$cache=$this->getCache();$key=$this->getKey();if(isset($cache[$key])){$this->list=$cache[$key];}else{$this->rebuild();}if(isset($this->list[strtolower(__CLASS__)])&&class_exists('NetteX\Loaders\NetteLoader',FALSE)){NetteLoader::getInstance()->unregister();}parent::register();}function
...
rebuild(){$this->getCache()->save($this->getKey(),callbackX($this,'_rebuildCallback'),array(Cache::CONSTS=>'NetteX\Framework::REVISION'));$this->rebuilt=TRUE;}function
...
new
NetteX\DirectoryNotFoundException("Directory '$val' not found.");}$this->scanDirs[]=$real;}return$this;}private
function
...
as$class=>$pair){if($pair&&$pair[0]===$file){unset($this->list[$class]);}}if($matches=Strings::match($s,'#//nette'.'loader=(\S*)#')){foreach(explode(',',$matches[1])as$name){$this->addClass($name,$file,$time);}return;}foreach(token_get_all($s)as$token){if(is_array($token)){switch($token[0]){case
T_COMMENT:case
T_DOC_COMMENT:case
...
as$email=>$name){if($name!==NULL&&!Strings::checkEncoding($name)){throw
new
...
new
NetteX\InvalidArgumentException("Name must not contain line separator.");}$tmp[$email]=$name;}}else{$value=(string)$value;if(!Strings::checkEncoding($value)){throw
new
...
instanceof
NetteX\Templating\ITemplate){$text->mail=$this;$this->setBody($text->__toString(TRUE));}elseif($text==NULL&&$this->html!=NULL){$text=Strings::replace($this->html,array('#<(style|script|head).*</\\1>#Uis'=>'','#<t[dh][ >]#i'=>" $0",'#[ \t\r\n]+#'=>' ','#<(/?p|/?h\d|li|br|/tr)[ >/]#i'=>"\n$0"));$text=html_entity_decode(strip_tags($text),ENT_QUOTES,'UTF-8');$this->setBody(trim($text));}}private
function
...
NetteX\Caching\Storages\DevNullStorage;}$outerCache=new
NetteX\Caching\Cache(self::$cacheStorage,'NetteX.Reflection.Annotations');if(self::$cache===NULL){self::$cache=(array)$outerCache->offsetGet('list');self::$timestamps=isset(self::$cache['*'])?self::$cache['*']:array();}if(!isset(self::$cache[$type])&&$file){self::$cache['*'][$file]=filemtime($file);self::parseScript($file);$outerCache->save('list',self::$cache);}if(isset(self::$cache[$type][$member])){$annotations=self::$cache[$type][$member];}else{$annotations=array();}}if($r
...
function
setCacheStorage(NetteX\Caching\IStorage$storage){self::$cacheStorage=$storage;}static
function
...
toCallback(){return
new
NetteX\Callback($this->value);}function
...
getClassName(){return($tmp=NetteX\Utils\Strings::match($this,'#>\s+([a-z0-9_\\\\]+)#i'))?$tmp[1]:NULL;}function
...
as$resource){$this->checkResource($resource);}}if($privileges===self::ALL){$privileges=array();}elseif(!is_array($privileges)){$privileges=array($privileges);}$assertion=$assertion?callbackX($assertion):NULL;if($toAdd){foreach($resources
...
NetteX\StaticClassException;}static
function
loader($helper){$callback=callbackX('NetteX\Templating\DefaultHelpers',$helper);if($callback->isCallable()){return$callback;}$callback=callbackX('NetteX\Utils\Strings',$helper);if($callback->isCallable()){return$callback;}}static
function
escapeHtml($s,$quotes=NULL){if(is_object($s)&&($s
instanceof
ITemplate||$s
instanceof
Html||$s
...
htmlSpecialChars($s,$quotes===''?ENT_NOQUOTES:($quotes==='"'?ENT_COMPAT:ENT_QUOTES));}static
function
...
function
escapeHtmlJs($s,$quotes=NULL){return
htmlSpecialChars(self::escapeJs($s),$quotes==='"'?ENT_COMPAT:ENT_QUOTES);}static
function
strip($s){return
...
registerFilter($callback){$callback=callbackX($callback);if(in_array($callback,$this->filters)){throw
new
...
registerHelper($name,$callback){$this->helpers[strtolower($name)]=callbackX($callback);}function
registerHelperLoader($callback){$this->helperLoaders[]=callbackX($callback);}final
function
...
Caching\Cache($storage=$this->getCacheStorage(),'NetteX.FileTemplate');if($storage
instanceof
Caching\Storages\PhpFileStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$content=$cache->load($this->file);if($content===NULL){try{$content=$this->compile(file_get_contents($this->file));$content="<?php\n\n// source file: $this->file\n\n?>$content";}catch(FilterException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$content,array(Caching\Cache::FILES=>$this->file,Caching\Cache::CONSTS=>'NetteX\Framework::REVISION'));$cache->release();$cached=$cache->load($this->file);}if($cached!==NULL&&$storage
instanceof
Caching\Storages\PhpFileStorage){NetteX\Utils\LimitedScope::load($cached['file'],$this->getParams());flock($cached['handle'],LOCK_UN);fclose($cached['handle']);}else{NetteX\Utils\LimitedScope::evaluate($content,$this->getParams());}}function
setCacheStorage(Caching\IStorage$storage){$this->cacheStorage=$storage;}function
getCacheStorage(){if($this->cacheStorage===NULL){return
new
Caching\Storages\DevNullStorage;}return$this->cacheStorage;}}}namespace NetteX\Utils{use
NetteX;final
class
Arrays{final
function
...
function
grep(array$arr,$pattern,$flags=0){NetteX\Diagnostics\Debugger::tryError();$res=preg_grep($pattern,$arr,$flags);Strings::catchPregError($pattern);return$res;}}final
class
...
NetteX\Utils\RecursiveDirectoryIteratorFixed($path);}else{$iterator=new\RecursiveDirectoryIterator($path,\RecursiveDirectoryIterator::FOLLOW_SYMLINKS);}if($this->exclude){$filters=$this->exclude;$iterator=new
...
getLength(){return$this->itemCount===NULL?$this->itemsPerPage:min($this->itemsPerPage,$this->itemCount-$this->getPageIndex()*$this->itemsPerPage);}}}namespace NetteX\Diagnostics{use
NetteX;final
class
Debugger{public
...
instanceof
NetteX\Templating\FilterException){return
...
new
NetteX\InvalidStateException('Logging directory is not specified in NetteX\Diagnostics\Debugger::$logDirectory.');}if($message
...
NetteX\FatalErrorException($error['message'],0,$error['type'],$error['file'],$error['line'],NULL));}if(self::$bar&&!self::$productionMode&&!self::$ajaxDetected&&!self::$consoleMode&&!preg_match('#^Content-Type: (?!text/html)#im',implode("\n",headers_list()))){self::$bar->render();}}static
function
...
function
dumpX($var,$return=FALSE){if(!$return&&self::$productionMode){return$var;}$output="<pre class=\"nette-dump\">".Helpers::htmlDump($var)."</pre>\n";if(!$return){$trace=debug_backtrace();$i=!isset($trace[1]['class'])&&isset($trace[1]['function'])&&$trace[1]['function']==='dump'?1:0;if(isset($trace[$i]['file'],$trace[$i]['line'])&&is_file($trace[$i]['file'])){$lines=file($trace[$i]['file']);preg_match('#dump\((.*)\)#',$lines[$trace[$i]['line']-1],$m);$output=substr_replace($output,' title="'.htmlspecialchars((isset($m[0])?"$m[0] \n":'')."in file {$trace[$i]['file']} on line {$trace[$i]['line']}").'"',4,0);if(self::$showLocation){$output=substr_replace($output,' <small>in '.Helpers::editorLink($trace[$i]['file'],$trace[$i]['line']).":{$trace[$i]['line']}</small>",-8,0);}}}if(self::$consoleMode){$output=htmlspecialchars_decode(strip_tags($output),ENT_NOQUOTES);}if($return){return$output;}else{echo$output;return$var;}}static
function
...
self::$fireLogger->log($message);}}static
function
