...
extends ReflectionBase implements IReflectionProperty{private$declaringClassName;private$modifiers=0;private$defaultValue;private$defaultValueDefinition=array();private$accessible=false;public
...
function isDefault(){return null!==$this->getDefaultValueDefinition();}public function
getModifiers(){return$this->modifiers;}public function isPrivate(){return (bool)($this->modifiers&InternalReflectionProperty::IS_PRIVATE);}public
...
isset($interfaces[$interfaceName]);}public function getInterfaces(){if(null===$this->interfaces){$broker=$this->broker;$this->interfaces=array_map(function($interfaceName)use($broker){return$broker->getClass($interfaceName);},$this->getInterfaceNames());}return$this->interfaces;}public
...
array_unique($names);}public function getOwnInterfaces(){$interfaceNames=$this->getOwnInterfaceNames();if(empty($interfaceNames)){return
array();}$broker=$this->getBroker();return array_combine($interfaceNames,array_map(function($interfaceName)use($broker){return$broker->getClass($interfaceName);},$interfaceNames));}public
function getOwnInterfaceNames(){return array_reverse($this->interfaces);}public function
getConstructor(){foreach($this->getMethods()as$method){if($method->isConstructor()){return$method;}}return
...
(bool)($property->getModifiers()&$filter);});}return array_values($properties);}public
function getDefaultProperties(){static$accessLevels=array(InternalReflectionProperty::IS_PUBLIC,InternalReflectionProperty::IS_PRIVATE,InternalReflectionProperty::IS_PROTECTED);$defaults=array();$properties=$this->getProperties();foreach(array(true,false)as$static){foreach($accessLevels
...
extends ReflectionBase implements IReflectionFunctionBase{protected$namespaceName;protected$modifiers=0;private$returnsReference=false;private$parameters=array();private$staticVariables=array();public
...
function getStaticVariables(){return$this->staticVariables;}final protected function
parseReturnsReference(Stream$tokenStream){try{if(!$tokenStream->is(T_FUNCTION)){throw
