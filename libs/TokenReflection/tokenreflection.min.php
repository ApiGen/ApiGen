...
T_STATIC:$type=$tokenStream->skipWhitespaces()->getType();if(T_DOUBLE_COLON===$type||'('===$type){break;}elseif(T_VARIABLE!==$type){throw
new Exception\Parse(sprintf('Invalid token found: "%s".',$tokenStream->getTokenName()),Exception\Parse::PARSE_CHILDREN_ERROR);}while(T_VARIABLE===$type){$variableName=$tokenStream->getTokenValue();$variableDefinition=array();$type=$tokenStream->skipWhitespaces()->getType();if('='===$type){$type=$tokenStream->skipWhitespaces()->getType();$level=0;while($tokenStream->valid()){switch($type){case
